...
You can deploy the frontend and backend containers with one command from the repository root.
...
```bash
npm run prod:ps      # service status
npm run prod:logs    # stream logs
npm run prod:down    # stop and remove containers
npm run prod:reset   # stop and remove containers + volumes
```
...
# - AI reporting credentials are configured per-tenant in Settings → AI Reporting
...
| `AI_REPORTING_ENABLED` | No | Feature flag for AI NLQ reporting (default `false`). Per-tenant provider/model/API key are stored in `ai_tenant_provider_configs` and managed in Settings → AI Reporting. |
| `AI_CONFIG_ENCRYPTION_KEY` | No | 32+ char key for encrypting tenant API keys at rest. Falls back to `JWT_SECRET` if unset. |
