...
const tenant_bootstrap_defaults_1 = require("../src/modules/auth/tenant-bootstrap.defaults");
const prisma = new client_1.PrismaClient();
function toLegacyCategoryCode(name) {
...
function generateSeasonalValue(baseValue, month, growthRate = 0.05) {
...
function normalizeTenantSlug(value, fallback) {
    ...
    if (!/^[a-z0-9-]+$/.test(normalized))
        return fallback;
    if (['www', 'api', 'app'].includes(normalized))
        return fallback;
    return normalized;
}
...
async function main() {
    ...
    const tenant = await prisma.tenant.upsert({
        ...
        update: {
            ...
            fiscalYearStart: 1,
            defaultCurrency: 'USD',
            settings: tenant_bootstrap_defaults_1.DEFAULT_TENANT_SETTINGS,
        },
        create: {
            name: seedTenant.tenantName,
    ...
    for (const u of tenant_bootstrap_defaults_1.DEFAULT_TENANT_UOMS) {
    ...
    console.log(`✅ Created ${tenant_bootstrap_defaults_1.DEFAULT_TENANT_UOMS.length} units of measure`);
    // ============================================
    // PRODUCT CATEGORIES — Standard categories
    // ============================================
    await prisma.productCategory.deleteMany({ where: { tenantId: tenant.id } });
    for (const c of tenant_bootstrap_defaults_1.DEFAULT_PRODUCT_CATEGORIES) {
    ...
    console.log(`✅ Created ${tenant_bootstrap_defaults_1.DEFAULT_PRODUCT_CATEGORIES.length} product categories`);
    // ============================================
    ...
    const extraProductCategories = [...new Set(productData.map((product) => product.category.trim()))]
        .filter((categoryName) => !tenant_bootstrap_defaults_1.DEFAULT_PRODUCT_CATEGORIES.some((category) => category.name.toLowerCase() === categoryName.toLowerCase()))
        ...
        sortOrder: tenant_bootstrap_defaults_1.DEFAULT_PRODUCT_CATEGORIES.length + index + 1,
        isActive: true,
    }));
    ...
    const productCategoryByName = new Map(productCategories.map((category) => [category.name.toLowerCase(), category]));
    ...
    // ============================================
    const locationData = [
        { code: 'NA-EAST-WH', name: 'East Coast Warehouse', region: 'North America', country: 'USA', city: 'New Jersey', type: client_1.LocationType.WAREHOUSE },
        { code: 'NA-WEST-WH', name: 'West Coast Warehouse', region: 'North America', country: 'USA', city: 'Los Angeles', type: client_1.LocationType.WAREHOUSE },
        { code: 'NA-CENTRAL-DC', name: 'Central Distribution', region: 'North America', country: 'USA', city: 'Chicago', type: client_1.LocationType.DISTRIBUTION_CENTER },
        { code: 'NA-NYC-STORE', name: 'NYC Flagship Store', region: 'North America', country: 'USA', city: 'New York', type: client_1.LocationType.STORE },
        { code: 'NA-SF-STORE', name: 'San Francisco Store', region: 'North America', country: 'USA', city: 'San Francisco', type: client_1.LocationType.STORE },
        { code: 'EU-UK-WH', name: 'UK Warehouse', region: 'Europe', country: 'UK', city: 'London', type: client_1.LocationType.WAREHOUSE },
        { code: 'EU-DE-DC', name: 'Germany Distribution', region: 'Europe', country: 'Germany', city: 'Frankfurt', type: client_1.LocationType.DISTRIBUTION_CENTER },
        { code: 'APAC-SG-WH', name: 'Singapore Hub', region: 'Asia Pacific', country: 'Singapore', city: 'Singapore', type: client_1.LocationType.WAREHOUSE },
        { code: 'APAC-JP-STORE', name: 'Tokyo Store', region: 'Asia Pacific', country: 'Japan', city: 'Tokyo', type: client_1.LocationType.STORE },
    ];
    const locations = [];
    ...
    // ============================================
    const customerData = [
        { code: 'CUST-ENT-001', name: 'Global Tech Corp', segment: 'Enterprise', industry: 'Technology', type: client_1.CustomerType.DIRECT, country: 'USA' },
        { code: 'CUST-ENT-002', name: 'Financial Services Inc', segment: 'Enterprise', industry: 'Finance', type: client_1.CustomerType.DIRECT, country: 'USA' },
        { code: 'CUST-ENT-003', name: 'Healthcare Systems', segment: 'Enterprise', industry: 'Healthcare', type: client_1.CustomerType.DIRECT, country: 'USA' },
        { code: 'CUST-MID-001', name: 'Regional Manufacturing', segment: 'Mid-Market', industry: 'Manufacturing', type: client_1.CustomerType.DISTRIBUTOR, country: 'USA' },
        { code: 'CUST-MID-002', name: 'Retail Chain Partners', segment: 'Mid-Market', industry: 'Retail', type: client_1.CustomerType.RETAILER, country: 'USA' },
        { code: 'CUST-MID-003', name: 'Education Solutions', segment: 'Mid-Market', industry: 'Education', type: client_1.CustomerType.DIRECT, country: 'UK' },
        { code: 'CUST-SMB-001', name: 'Local Business Group', segment: 'SMB', industry: 'Professional Services', type: client_1.CustomerType.DIRECT, country: 'USA' },
        { code: 'CUST-SMB-002', name: 'Startup Ventures', segment: 'SMB', industry: 'Technology', type: client_1.CustomerType.DIRECT, country: 'Germany' },
        { code: 'CUST-ECOM-001', name: 'Online Marketplace', segment: 'E-Commerce', industry: 'E-Commerce', type: client_1.CustomerType.ECOMMERCE, country: 'Global' },
        { code: 'CUST-WHOLE-001', name: 'Wholesale Distributors', segment: 'Wholesale', industry: 'Distribution', type: client_1.CustomerType.WHOLESALE, country: 'USA' },
    ];
    const customers = [];
    ...
    // ============================================
    const accountData = [
        { code: '4000', name: 'Total Revenue', type: client_1.AccountType.REVENUE, category: 'Revenue' },
        { code: '4100', name: 'Product Revenue', type: client_1.AccountType.REVENUE, category: 'Revenue' },
        { code: '4200', name: 'Service Revenue', type: client_1.AccountType.REVENUE, category: 'Revenue' },
        { code: '4300', name: 'Software License Revenue', type: client_1.AccountType.REVENUE, category: 'Revenue' },
        { code: '5000', name: 'Total Cost of Sales', type: client_1.AccountType.COST_OF_GOODS, category: 'Cost of Sales' },
        { code: '5100', name: 'Product COGS', type: client_1.AccountType.COST_OF_GOODS, category: 'Cost of Sales' },
        { code: '5200', name: 'Service Delivery Cost', type: client_1.AccountType.COST_OF_GOODS, category: 'Cost of Sales' },
        { code: '6000', name: 'Operating Expenses', type: client_1.AccountType.OPERATING_EXPENSE, category: 'OpEx' },
        { code: '6100', name: 'Sales & Marketing', type: client_1.AccountType.OPERATING_EXPENSE, category: 'OpEx' },
        { code: '6200', name: 'R&D Expenses', type: client_1.AccountType.OPERATING_EXPENSE, category: 'OpEx' },
        { code: '6300', name: 'G&A Expenses', type: client_1.AccountType.OPERATING_EXPENSE, category: 'OpEx' },
    ];
    const accounts = [];
        ...
        for (const product of products) {
            ...
            for (const location of selectedLocations) {
                ...
                for (const customer of selectedCustomers) {
                    ...
                    actualsBatch.push({
                        tenantId: tenant.id,
                        actualType: client_1.ActualType.SALES,
    ...
    const plan2025 = await prisma.planVersion.create({
        data: {
            ...
            planType: client_1.PlanType.BUDGET,
            status: client_1.PlanStatus.APPROVED,
            version: 1,
    ...
    const plan2026 = await prisma.planVersion.create({
        data: {
            ...
            planType: client_1.PlanType.FORECAST,
            status: client_1.PlanStatus.DRAFT,
            version: 1,
    ...
    const planQ1 = await prisma.planVersion.create({
        data: {
            ...
            planType: client_1.PlanType.WHAT_IF,
            status: client_1.PlanStatus.IN_REVIEW,
            version: 1,
            startDate: new Date('2026-01-01'),
    ...
    const scenario2025Base = await prisma.scenario.create({
        data: {
            ...
            scenarioType: client_1.ScenarioType.BASE,
            isBaseline: true,
        },
    });
    const scenario2025Optimistic = await prisma.scenario.create({
        data: {
            ...
            scenarioType: client_1.ScenarioType.OPTIMISTIC,
            isBaseline: false,
        },
    });
    const scenario2025Conservative = await prisma.scenario.create({
        data: {
            ...
            scenarioType: client_1.ScenarioType.PESSIMISTIC,
            isBaseline: false,
        },
    });
    ...
    const forecastModels = [
        { model: client_1.ForecastModel.HOLT_WINTERS, variance: 1.0 },
        { model: client_1.ForecastModel.MOVING_AVERAGE, variance: 0.92 },
        { model: client_1.ForecastModel.LINEAR_REGRESSION, variance: 1.05 },
        { model: client_1.ForecastModel.AI_HYBRID, variance: 1.02 },
    ];
    const allScenarios = [
                ...
                for (const location of locations.slice(0, 4)) {
                    ...
                    for (const { model, variance } of forecastModels) {
                        ...
                        forecastBatch.push({
                            ...
                            currency: 'USD',
                            forecastModel: model,
                            confidenceLevel: model === client_1.ForecastModel.AI_HYBRID ? 90 : 85,
                            createdById: adminUser.id,
                        });
                        forecastCount++;
                    }
    ...
    for (let year = 2024; year <= 2027; year++) {
        ...
        for (let q = 0; q < 4; q++) {
            ...
            await prisma.timeBucket.create({
                data: {
                    tenantId: tenant.id,
                    periodType: client_1.PeriodType.QUARTERLY,
        ...
        await prisma.timeBucket.create({
            data: {
                tenantId: tenant.id,
                periodType: client_1.PeriodType.YEARLY,
                periodKey: `${year}`,
        ...
        while (weekDate.getFullYear() === year) {
            ...
            await prisma.timeBucket.create({
                data: {
                    tenantId: tenant.id,
                    periodType: client_1.PeriodType.WEEKLY,
    ...
    for (const { scenario, plan, growthRate } of allScenarios) {
        ...
        const forecastRun = await prisma.forecastRun.create({
            data: {
                ...
                planVersionId: plan.id,
                scenarioId: scenario.id,
                forecastModel: client_1.ForecastModel.HOLT_WINTERS,
                modelVersion: '1.0.0',
                isPersistent: true,
    ...
    const getProduct = (code) => products.find(p => p.code === code);
    const getLocation = (code) => locations.find(l => l.code === code);
    // ============================================
    // WORK CENTERS - Production equipment and labor pools
    // ============================================
    const workCenterData = [
        { code: 'WC-SMT-01', name: 'SMT Assembly Line 1', type: client_1.WorkCenterType.MACHINE, costPerHour: 150, efficiency: 95, description: 'Surface mount technology for PCB assembly' },
        { code: 'WC-SMT-02', name: 'SMT Assembly Line 2', type: client_1.WorkCenterType.MACHINE, costPerHour: 150, efficiency: 92, description: 'Surface mount technology backup line' },
        { code: 'WC-ASSY-01', name: 'Final Assembly Station 1', type: client_1.WorkCenterType.MIXED, costPerHour: 80, efficiency: 90, description: 'Final product assembly with manual workers' },
        { code: 'WC-ASSY-02', name: 'Final Assembly Station 2', type: client_1.WorkCenterType.MIXED, costPerHour: 80, efficiency: 88, description: 'Final product assembly backup' },
        { code: 'WC-TEST-01', name: 'Quality Testing Station', type: client_1.WorkCenterType.MACHINE, costPerHour: 100, efficiency: 98, description: 'Automated testing and QA' },
        { code: 'WC-PACK-01', name: 'Packaging Line 1', type: client_1.WorkCenterType.LABOR, costPerHour: 45, efficiency: 95, description: 'Product packaging and boxing' },
        { code: 'WC-BURN-01', name: 'Burn-in Testing', type: client_1.WorkCenterType.MACHINE, costPerHour: 60, efficiency: 99, description: '48-hour burn-in testing chamber' },
        { code: 'WC-LCD-01', name: 'LCD Assembly Line', type: client_1.WorkCenterType.MACHINE, costPerHour: 120, efficiency: 94, description: 'Monitor panel assembly' },
    ];
    const workCenters = [];
    ...
    for (const wc of workCenterData) {
        const workCenter = await prisma.workCenter.create({
            data: {
                ...
                utilization: 85,
                status: client_1.DimensionStatus.ACTIVE,
            },
        });
        ...
        await prisma.workCenterCapacity.create({
            data: {
                ...
                capacityPerDay: wc.type === client_1.WorkCenterType.MACHINE ? 480 : 240, // 8 hours * 60 min for machines, less for labor
                capacityUOM: 'MIN',
                numberOfMachines: wc.type === client_1.WorkCenterType.MACHINE ? 2 : 1,
                numberOfShifts: 2,
                hoursPerShift: 8,
            },
        });
    ...
    if (laptopPro) {
        const bomLaptopPro = await prisma.billOfMaterial.create({
            data: {
                ...
                version: '1.0',
                type: client_1.BOMType.MANUFACTURING,
                status: client_1.BOMStatus.ACTIVE,
                effectiveFrom: new Date('2024-01-01'),
                baseQuantity: 1,
                baseUOM: 'EA',
            ...
            if (compProduct) {
                await prisma.bOMComponent.create({
                    data: {
                        ...
                        supplyType: client_1.SupplyType.STOCK,
    ...
    // ============================================
    const getWorkCenter = (code) => workCenters.find(wc => wc.code === code);
    ...
    // ============================================
    let policyCount = 0;
    const warehouseLocations = locations.filter(l => l.type === client_1.LocationType.WAREHOUSE);
        ...
        for (const location of warehouseLocations) {
            await prisma.inventoryPolicy.create({
                data: {
                    ...
                    productId: product.id,
                    locationId: location.id,
                    planningMethod: client_1.PlanningMethod.MRP,
                    lotSizingRule: client_1.LotSizingRule.FOQ,
                    safetyStockMethod: client_1.SafetyStockMethod.DAYS_OF_SUPPLY,
                    safetyStockDays: 7,
                    serviceLevel: 95,
                    reorderPoint: 50,
                    reorderQty: 100,
        ...
        for (const location of warehouseLocations.slice(0, 2)) { // Only primary warehouses
            ...
            await prisma.inventoryPolicy.create({
                data: {
                    ...
                    productId: product.id,
                    locationId: location.id,
                    planningMethod: client_1.PlanningMethod.REORDER_POINT,
                    lotSizingRule: client_1.LotSizingRule.EOQ,
                    safetyStockMethod: client_1.SafetyStockMethod.FIXED,
        ...
        for (const location of warehouseLocations.slice(0, 2)) {
            await prisma.inventoryPolicy.create({
                data: {
                    ...
                    productId: product.id,
                    locationId: location.id,
                    planningMethod: client_1.PlanningMethod.MIN_MAX,
                    lotSizingRule: client_1.LotSizingRule.MIN_MAX,
                    safetyStockMethod: client_1.SafetyStockMethod.FIXED,
                    safetyStockQty: 500,
                    minOrderQty: 100,
                    maxOrderQty: 5000,
    ...
    const workflowTemplates = [
        {
            ...
            entityType: client_1.WorkflowEntityType.PLAN_VERSION,
            steps: [
                { sequence: 1, name: 'Manager Review', approverType: client_1.ApproverType.ROLE, approverRole: client_1.UserRole.ADMIN, requiredApprovals: 1, canReject: true, canDelegate: true },
            ],
        },
        {
            name: 'Forecast Override Approval',
            description: 'Approval workflow for manual forecast overrides',
            entityType: client_1.WorkflowEntityType.FORECAST_OVERRIDE,
            steps: [
                { sequence: 1, name: 'Planner Review', approverType: client_1.ApproverType.ROLE, approverRole: client_1.UserRole.PLANNER, requiredApprovals: 1, canReject: true, canDelegate: false },
                { sequence: 2, name: 'Finance Approval', approverType: client_1.ApproverType.ROLE, approverRole: client_1.UserRole.FINANCE, requiredApprovals: 1, canReject: true, canDelegate: true },
            ],
        },
        {
            name: 'BOM Change Approval',
            description: 'Quality gate for bill of material changes',
            entityType: client_1.WorkflowEntityType.BOM,
            steps: [
                { sequence: 1, name: 'Engineering Review', approverType: client_1.ApproverType.ROLE, approverRole: client_1.UserRole.PLANNER, requiredApprovals: 1, canReject: true, canDelegate: false },
                { sequence: 2, name: 'Production Head Approval', approverType: client_1.ApproverType.ROLE, approverRole: client_1.UserRole.ADMIN, requiredApprovals: 1, canReject: true, canDelegate: true },
            ],
        },
        {
            name: 'Promotion Approval',
            description: 'Multi-step approval for promotional plans',
            entityType: client_1.WorkflowEntityType.PROMOTION,
            steps: [
                { sequence: 1, name: 'Brand Manager Review', approverType: client_1.ApproverType.ROLE, approverRole: client_1.UserRole.PLANNER, requiredApprovals: 1, canReject: true, canDelegate: false },
                { sequence: 2, name: 'Finance Sign-off', approverType: client_1.ApproverType.ROLE, approverRole: client_1.UserRole.FINANCE, requiredApprovals: 1, canReject: true, canDelegate: true },
            ],
        },
        {
            name: 'Scenario Approval',
            description: 'Approval for scenarios before publishing',
            entityType: client_1.WorkflowEntityType.SCENARIO,
            steps: [
                { sequence: 1, name: 'Lead Planner Approval', approverType: client_1.ApproverType.ROLE, approverRole: client_1.UserRole.ADMIN, requiredApprovals: 1, canReject: true, canDelegate: true },
            ],
        },
        {
            name: 'Planned Order Release',
            description: 'Approval workflow for MRP planned order release',
            entityType: client_1.WorkflowEntityType.PLANNED_ORDER,
            steps: [
                { sequence: 1, name: 'Production Planner Approval', approverType: client_1.ApproverType.ROLE, approverRole: client_1.UserRole.PLANNER, requiredApprovals: 1, canReject: true, canDelegate: false },
            ],
        },
    ];
