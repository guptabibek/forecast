  ...
  imports: [
    ...
    ThrottlerModule.forRootAsync({
      ...
      useFactory: (config: ConfigService) => [
        {
          name: 'short',
          ttl: positiveInt(config.get('THROTTLE_SHORT_TTL_MS'), 1000),
          limit: positiveInt(config.get('THROTTLE_SHORT_LIMIT'), 100),
        },
        {
          name: 'medium',
          ttl: positiveInt(config.get('THROTTLE_MEDIUM_TTL_MS'), 10000),
          limit: positiveInt(config.get('THROTTLE_MEDIUM_LIMIT'), 500),
        },
        {
          name: 'long',
          ttl: positiveInt(config.get('THROTTLE_LONG_TTL_MS'), 60000),
          limit: positiveInt(config.get('THROTTLE_LONG_LIMIT'), 3000),
        },
      ],
    }),
