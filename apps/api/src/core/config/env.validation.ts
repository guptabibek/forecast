...
function parsePositiveInt(env: RawEnv, key: string, fallback: number, max?: number): number {
  const raw = env[key]?.trim();
  if (!raw) return fallback;
  const parsed = Number(raw);
  if (!Number.isInteger(parsed) || parsed < 1 || (max !== undefined && parsed > max)) {
    throw new Error(`Environment validation failed: ${key} must be a positive integer${max ? ` up to ${max}` : ''}`);
  }
  return parsed;
}
...
export function validateEnv(env: RawEnv): ValidatedEnv {
  ...
  // AI reporting feature: gated per-tenant via tenant_modules (module key
  // 'ai-reporting', controlled by superadmin). All operational config —
  // timeouts, limits, rate caps, feature flags — lives in
  // ai_tenant_provider_configs and is managed by the tenant admin. The only
  // env var that remains is the optional encryption key for stored API keys.
  if (env.AI_CONFIG_ENCRYPTION_KEY?.trim()) {
    assertMinLength('AI_CONFIG_ENCRYPTION_KEY', env.AI_CONFIG_ENCRYPTION_KEY.trim(), 32);
    assertNoPlaceholderInProd('AI_CONFIG_ENCRYPTION_KEY', env.AI_CONFIG_ENCRYPTION_KEY.trim(), nodeEnv);
  }
