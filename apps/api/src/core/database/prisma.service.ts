...
export class PrismaService extends PrismaClient implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(private readonly cls: ClsService) {
    ...
    this.$use(async (params, next) => {
      ...
      const tenantId = this.getTenantId();
      const allowUnscopedTenantQuery = this.getAllowUnscopedTenantQuery();
      if (!tenantId || tenantId === SA_SYNTHETIC_TENANT_ID) {
        if (allowUnscopedTenantQuery) {
          return next(params);
        }
  ...
  private getAllowUnscopedTenantQuery(): boolean {
    try {
      return this.cls.isActive() ? Boolean(this.cls.get(this.allowUnscopedTenantQueryKey)) : false;
    } catch {
      return false;
    }
  }
  ...
  /**
   * Execute an intentional tenant-agnostic lookup without emitting the strict
   * tenant-context warning. Use sparingly for bootstrapping tenant resolution.
   */
  async executeWithoutTenantScopeWarning<T>(callback: () => Promise<T>): Promise<T> {
    return this.cls.run(async () => {
      this.cls.set(this.allowUnscopedTenantQueryKey, true);
      return callback();
    });
  }
}
