...
export class UsersService {
  ...
  private static readonly USER_ALLOWED_FIELDS = {
    email: 'string',
    firstName: 'string',
    lastName: 'string',
    role: 'enum',
  ...
  async findAll(query: UserQueryDto, currentUser: any) {
    ...
    return applyGridQuery<any>(this.prisma.user, query, {
      baseWhere,
      allowedFields: UsersService.USER_ALLOWED_FIELDS,
      defaultOrderBy: { createdAt: 'desc' },
      defaultPageSize: query.limit ?? 100,
      select: {
        id: true,
        email: true,
        firstName: true,
  ...
  private buildWorkspaceUrl(tenant: {
    ...
    const configuredWebPort = (this.configService.get<string>('WEB_PORT') || '3000').trim();
    const localWebPort = configuredWebPort.split(':').pop() || '3000';
    ...
    if (mainDomain) {
      const portSuffix = mainDomain === 'localhost' ? `:${localWebPort}` : '';
      return `${protocol}//${workspaceSlug}.${mainDomain}${portSuffix}`;
    }

    // Local development fallback
    return `http://${workspaceSlug}.localhost:${localWebPort}`;
  }
