...
import { buildPrismaFilterAnd, buildPrismaOrderBy, parseFiltersParam } from '../../core/filtering/filter.util';
...
export class DataService {
  ...
  generateTemplate(type: string): string {
    const templates: Record<string, string> = {
      ...
      products: 'code,name,description,category,product_company,salt,product_group,hsn_code,list_price\nPROD-001,Product 1,Description,Electronics,Company A,Salt A,Group A,30049099,99.99',
    ...
    const templateDefinitions: Record<string, {
      ...
      products: {
        columns: [
          ...
          { name: 'product_company', required: false, type: 'string', description: 'Product company / manufacturer' },
          { name: 'salt', required: false, type: 'string', description: 'Product salt / generic composition' },
          { name: 'product_group', required: false, type: 'string', description: 'Product group' },
          { name: 'hsn_code', required: false, type: 'string', description: 'HSN code' },
          { name: 'list_price', required: false, type: 'number', description: 'Standard list price' },
        ],
        sampleData: [
          { code: 'PROD-001', name: 'Widget A', description: 'Premium widget', category: 'Electronics', product_company: 'Company A', salt: 'Salt A', product_group: 'Group A', hsn_code: '30049099', list_price: '99.99' },
          { code: 'PROD-002', name: 'Gadget B', description: 'Standard gadget', category: 'Hardware', product_company: 'Company B', salt: 'Salt B', product_group: 'Group B', hsn_code: '30049091', list_price: '149.99' },
        ],
      },
      locations: {
        columns: [
  ...
  private async buildProductSpecificData(
    ...
    if (dto.productCompany !== undefined) {
      specificData.productCompany = this.normalizeOptionalString(dto.productCompany) ?? null;
    }

    if (dto.salt !== undefined) {
      specificData.salt = this.normalizeOptionalString(dto.salt) ?? null;
    }

    if (dto.productGroup !== undefined) {
      specificData.productGroup = this.normalizeOptionalString(dto.productGroup) ?? null;
    }

    if (dto.hsnCode !== undefined) {
      specificData.hsnCode = this.normalizeOptionalString(dto.hsnCode) ?? null;
    }
  ...
  private static readonly DIMENSION_ALLOWED_FIELDS: Record<string, AllowedFields> = {
    product: {
      code: 'string',
      name: 'string',
      description: 'string',
  ...
  async getDimensions(type: string, query: DimensionQueryDto, user: any) {
    ...
    const { search, isActive, page, pageSize, limit, sortBy, sortDir, filters: filtersRaw } = query;

    const take = limit || pageSize || 100;
    const skip = page ? (page - 1) * take : 0;

    const columnFilters = parseFiltersParam(filtersRaw);
    const allowedFields = DataService.DIMENSION_ALLOWED_FIELDS[type] ?? {};
    const filterAnd = buildPrismaFilterAnd(columnFilters, allowedFields);
    ...
    const where = filterAnd.length ? { AND: [baseWhere, ...filterAnd] } : baseWhere;

    const fallbackOrderBy = { name: 'asc' as const };
    const orderBy = buildPrismaOrderBy(sortBy, sortDir, allowedFields, fallbackOrderBy);

    const results = await model.findMany({ where, orderBy, take, skip });
