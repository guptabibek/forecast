...
export class PlatformController {
  ...
  @ApiOperation({ summary: 'Create a tenant user as Super Admin' })
  async createTenantUser(
    ...
    body: {
      ...
      firstName?: string;
      lastName?: string;
      role?: 'ADMIN' | 'PLANNER' | 'FINANCE' | 'VIEWER' | 'FORECAST_PLANNER' | 'FORECAST_VIEWER';
    },
  ) {
    return { data: await this.platformService.createTenantUser(id, body) };
  }

  @Patch('tenants/:tenantId/users/:userId')
  @ApiOperation({ summary: 'Update a tenant user as Super Admin' })
  async updateTenantUser(
    ...
    return { data: await this.platformService.updateTenantUser(tenantId, userId, body) };
  }

  @Delete('tenants/:tenantId/users/:userId')
  @HttpCode(HttpStatus.NO_CONTENT)
  @ApiOperation({ summary: 'Deactivate a tenant user as Super Admin' })
  async deactivateTenantUser(
    @Param('tenantId', ParseUUIDPipe) tenantId: string,
    @Param('userId', ParseUUIDPipe) userId: string,
  ) {
    await this.platformService.deactivateTenantUser(tenantId, userId);
  }
