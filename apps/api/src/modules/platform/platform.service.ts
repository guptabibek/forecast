...
export class PlatformService {
  ...
  async createTenantUser(
    ...
    if (role === UserRole.SUPER_ADMIN) {
      throw new BadRequestException('Tenant users cannot be created as SUPER_ADMIN');
    }

    const email = data.email?.trim().toLowerCase();
    if (!email || !data.password || data.password.length < 8) {
      throw new BadRequestException('Valid email and password of at least 8 characters are required');
    }
    ...
    const adminRole = role === UserRole.ADMIN
      ? await this.rolesService.ensureDefaultAdminRole(tenantId)
      : null;
    ...
    return this.prisma.user.create({
      data: {
        ...
        role,
        ...(adminRole ? { customRole: { connect: { id: adminRole.id } } } : {}),
  ...
  async updateTenantUser(
    ...
    if (data.role === UserRole.SUPER_ADMIN) {
      throw new BadRequestException('Tenant users cannot be elevated to SUPER_ADMIN');
    }
    ...
    if (data.role !== undefined) {
      ...
      if (data.role === UserRole.ADMIN) {
        const adminRole = await this.rolesService.ensureDefaultAdminRole(tenantId);
        updateData.customRole = { connect: { id: adminRole.id } };
      }
    }
