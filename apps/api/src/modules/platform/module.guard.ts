...
export class ModuleGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    const requiredModule = this.reflector.getAllAndOverride<
      PlatformModuleKey | PlatformModuleKey[]
    ...
    // Any-of semantics when an array is supplied: allow if at least one is enabled.
    const required = Array.isArray(requiredModule) ? requiredModule : [requiredModule];
    const checks = await Promise.all(required.map((m) => this.isModuleEnabled(tenantId, m)));
    if (!checks.some(Boolean)) {
      const list = required.join("', '");
      throw new ForbiddenException(
        `None of the required modules ('${list}') are enabled for your organization. Contact your administrator.`,
      );
    }
