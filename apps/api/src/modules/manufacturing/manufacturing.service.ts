...
import { applyGridQuery } from '../../core/filtering/grid-query.helper';
import type { AllowedFields } from '../../core/filtering/filter.types';
...
export class ManufacturingService {
  ...
  private static readonly BOM_ALLOWED_FIELDS: AllowedFields = {
    name: 'string',
    version: 'string',
    status: 'enum',
    type: 'enum',
  ...
  async getBOMs(tenantId: string, params: {
    ...
    const { status, productId, search } = params;

    const result = await applyGridQuery<any>(this.prisma.billOfMaterial, params, {
      baseWhere: {
        tenantId,
        ...(status && { status }),
      ...
      allowedFields: ManufacturingService.BOM_ALLOWED_FIELDS,
    ...
    result.items.forEach((bom: any) => {
      productIds.add(bom.parentProductId);
      bom.components.forEach((c: any) => productIds.add(c.componentProductId));
    });
    ...
    return {
      ...result,
      items: result.items.map((bom: any) => this.mapBOMResponse(bom, productMap)),
    };
  }
  ...
  private static readonly WORK_CENTER_ALLOWED_FIELDS = {
    code: 'string',
    name: 'string',
    description: 'string',
    type: 'enum',
  ...
  async getWorkCenters(tenantId: string, params: {
    ...
    const { type, isActive, locationId } = params;

    return applyGridQuery<any>(this.prisma.workCenter, params, {
      baseWhere: {
        tenantId,
        ...(type && { type }),
      ...
      allowedFields: ManufacturingService.WORK_CENTER_ALLOWED_FIELDS,
  ...
  private static readonly INVENTORY_POLICY_ALLOWED_FIELDS: AllowedFields = {
    planningMethod: 'enum',
    lotSizingRule: 'enum',
    safetyStockMethod: 'enum',
  ...
  async getInventoryPolicies(tenantId: string, params: {
    ...
    const result = await applyGridQuery<any>(this.prisma.inventoryPolicy, params, {
      baseWhere: {
        tenantId,
        ...(productId && { productId }),
        ...(locationId && { locationId }),
      },
      allowedFields: ManufacturingService.INVENTORY_POLICY_ALLOWED_FIELDS,
      defaultOrderBy: { createdAt: 'desc' },
    });

    const productIds = [...new Set(result.items.map((i: any) => i.productId))];
    const locationIds = [...new Set(result.items.map((i: any) => i.locationId))];
    const [products, locations] = await Promise.all([
      this.prisma.product.findMany({ where: { id: { in: productIds } }, select: { id: true, code: true, name: true } }),
      this.prisma.location.findMany({ where: { id: { in: locationIds } }, select: { id: true, code: true, name: true } }),
    ]);
    const productMap = new Map(products.map(p => [p.id, p]));
  ...
  private static readonly INVENTORY_LEVEL_ALLOWED_FIELDS: AllowedFields = {
    onHandQty: 'number',
    allocatedQty: 'number',
    inTransitQty: 'number',
    onOrderQty: 'number',
  ...
  async getInventoryLevels(tenantId: string, params: {
    ...
    const result = await applyGridQuery<any>(this.prisma.inventoryLevel, params, {
      baseWhere: {
        tenantId,
        ...(productId && { productId }),
        ...(locationId && { locationId }),
      },
      allowedFields: ManufacturingService.INVENTORY_LEVEL_ALLOWED_FIELDS,
  ...
  private static readonly PLANNED_ORDER_ALLOWED_FIELDS: AllowedFields = {
    orderType: 'enum',
    status: 'enum',
    quantity: 'number',
    dueDate: 'date',
  ...
  async getPlannedOrders(tenantId: string, params: {
    ...
    const { status, orderType, productId, locationId, startDate, endDate } = params;

    const result = await applyGridQuery<any>(this.prisma.plannedOrder, params, {
      baseWhere: {
        tenantId,
        ...(status && { status }),
        ...(orderType && { orderType }),
      ...
      allowedFields: ManufacturingService.PLANNED_ORDER_ALLOWED_FIELDS,
  ...
  private static readonly MRP_RUN_ALLOWED_FIELDS: AllowedFields = {
    name: 'string',
    runType: 'enum',
    status: 'enum',
  ...
  async getMRPRuns(tenantId: string, params: {
    ...
    const { runType, status } = params;
    return applyGridQuery<any>(this.prisma.mRPRun, params, {
      baseWhere: {
        tenantId,
        ...(runType && { runType }),
        ...(status && { status }),
      },
      allowedFields: ManufacturingService.MRP_RUN_ALLOWED_FIELDS,
      defaultOrderBy: { createdAt: 'desc' },
    });
  }
  ...
  private static readonly MRP_EXCEPTION_ALLOWED_FIELDS: AllowedFields = {
    exceptionType: 'enum',
    severity: 'enum',
    status: 'enum',
    message: 'string',
  ...
  async getMRPExceptions(tenantId: string, params: {
    ...
    const { status, exceptionType, severity, productId } = params;
    return applyGridQuery<any>(this.prisma.mRPException, params, {
      baseWhere: {
        mrpRun: { tenantId },
        ...(status && { status }),
      ...
      allowedFields: ManufacturingService.MRP_EXCEPTION_ALLOWED_FIELDS,
      defaultOrderBy: { createdAt: 'desc' },
    });
  }
  ...
  private static readonly SOP_CYCLE_ALLOWED_FIELDS: AllowedFields = {
    name: 'string',
    fiscalYear: 'number',
    fiscalPeriod: 'number',
    status: 'enum',
  ...
  async getSOPCycles(tenantId: string, params: {
    ...
    return applyGridQuery<any>(this.prisma.sOPCycle, params, {
      baseWhere: {
        tenantId,
        ...(year && { fiscalYear: year }),
        ...(status && { status }),
      },
      allowedFields: ManufacturingService.SOP_CYCLE_ALLOWED_FIELDS,
      defaultOrderBy: { fiscalYear: 'desc' },
      include: {
        _count: { select: { forecasts: true, assumptions: true } },
        demandManagerUser: { select: { id: true, firstName: true, lastName: true, email: true } },
        supplyManagerUser: { select: { id: true, firstName: true, lastName: true, email: true } },
        financeManagerUser: { select: { id: true, firstName: true, lastName: true, email: true } },
        executiveSponsorUser: { select: { id: true, firstName: true, lastName: true, email: true } },
      },
    });
  }
  ...
  private static readonly SUPPLIER_ALLOWED_FIELDS: AllowedFields = {
    code: 'string',
    name: 'string',
    contactName: 'string',
    email: 'string',
  ...
  async getSuppliers(tenantId: string, params: {
    ...
    const { search, isActive, isPreferred } = params;

    const result = await applyGridQuery<any>(this.prisma.supplier, params, {
      baseWhere: {
        tenantId,
        ...(search && {
          OR: [
      ...
      allowedFields: ManufacturingService.SUPPLIER_ALLOWED_FIELDS,
      defaultOrderBy: { name: 'asc' },
    });

    const normalizedItems = result.items.map((supplier: any) => {
  ...
  private static readonly PROMOTION_ALLOWED_FIELDS = {
    name: 'string',
    description: 'string',
    type: 'enum',
    status: 'enum',
  ...
  async getPromotions(tenantId: string, params: any) {
    ...
    const result = await applyGridQuery<any>(this.prisma.promotion, params, {
      baseWhere,
      allowedFields: ManufacturingService.PROMOTION_ALLOWED_FIELDS,
    ...
    return {
      ...result,
      items: this.serializePromotions(result.items),
    };
  }
  ...
  private static readonly NPI_ALLOWED_FIELDS: AllowedFields = {
    status: 'enum',
  ...
  async getNPIs(tenantId: string, params: any) {
    ...
    const result = await applyGridQuery<any>(this.prisma.newProductIntroduction, params, {
      baseWhere: { tenantId, ...(status && { status }) },
      allowedFields: ManufacturingService.NPI_ALLOWED_FIELDS,
      defaultOrderBy: { createdAt: 'desc' },
    });

    const productIds = result.items.map((item: any) => item.productId);
    const products = await this.prisma.product.findMany({
      where: { id: { in: productIds } },
  ...
  private static readonly WORKFLOW_TEMPLATE_ALLOWED_FIELDS = {
    name: 'string',
    description: 'string',
    entityType: 'enum',
    isActive: 'boolean',
  ...
  async getWorkflowTemplates(tenantId: string, params: any) {
    const { entityType, isActive } = params ?? {};
    return applyGridQuery<any>(this.prisma.workflowTemplate, params, {
      baseWhere: {
        tenantId,
        ...(entityType && { entityType }),
        ...(isActive !== undefined && { isActive }),
      },
      allowedFields: ManufacturingService.WORKFLOW_TEMPLATE_ALLOWED_FIELDS,
      defaultOrderBy: { createdAt: 'desc' },
  ...
  private static readonly WORKFLOW_INSTANCE_ALLOWED_FIELDS = {
    status: 'enum',
    entityType: 'enum',
    entityId: 'string',
    currentStep: 'number',
  ...
  async getWorkflowInstances(tenantId: string, params: any) {
    const { status, entityType, requestedById } = params ?? {};
    return applyGridQuery<any>(this.prisma.workflowInstance, params, {
      baseWhere: {
        tenantId,
        ...(status && { status }),
        ...(entityType && { entityType }),
      ...
      allowedFields: ManufacturingService.WORKFLOW_INSTANCE_ALLOWED_FIELDS,
  ...
  private static readonly PURCHASE_ORDER_ALLOWED_FIELDS: AllowedFields = {
    orderNumber: 'string',
    supplierId: 'string',
    status: 'enum',
    orderDate: 'date',
  ...
  async getPurchaseOrders(tenantId: string, filters?: {
    ...
    if (filters?.status) {
      baseWhere.status = this.mapApiStatusToPrismaStatus(filters.status);
    }
    if (filters?.supplierId) {
      baseWhere.supplierId = filters.supplierId;
    }
    if (filters?.startDate || filters?.endDate) {
      baseWhere.orderDate = {};
      if (filters.startDate) baseWhere.orderDate.gte = new Date(filters.startDate);
      if (filters.endDate) baseWhere.orderDate.lte = new Date(filters.endDate);
    }

    const result = await applyGridQuery<any>(this.prisma.purchaseOrder, filters, {
      baseWhere,
      allowedFields: ManufacturingService.PURCHASE_ORDER_ALLOWED_FIELDS,
      defaultOrderBy: { orderDate: 'desc' },
      include: { lines: true, receipts: true },
    });

    const supplierIds = [...new Set(result.items.map((po: any) => po.supplierId))];
    const suppliers = supplierIds.length > 0
      ? await this.prisma.supplier.findMany({ where: { id: { in: supplierIds } }, select: { id: true, name: true } })
      : [];
    const supplierMap = new Map(suppliers.map(s => [s.id, s]));

    const allProductIds = [...new Set(result.items.flatMap((po: any) => po.lines.map((l: any) => l.productId)))];
    const products = allProductIds.length > 0
      ? await this.prisma.product.findMany({ where: { id: { in: allProductIds } }, select: { id: true, code: true, name: true, unitOfMeasure: true } })
      : [];
    const productMap = new Map(products.map(p => [p.id, p as any]));

    return {
      ...result,
      items: result.items.map((po: any) => this.mapPurchaseOrderResponse(po, supplierMap.get(po.supplierId), productMap)),
    };
  }
  ...
  private static readonly WORK_ORDER_ALLOWED_FIELDS: AllowedFields = {
    orderNumber: 'string',
    status: 'enum',
    priority: 'enum',
    quantity: 'number',
  ...
  async getWorkOrders(tenantId: string, filters?: {
    ...
    if (filters?.workCenterId) baseWhere.workCenterId = filters.workCenterId;
    if (filters?.startDate || filters?.endDate) {
      baseWhere.plannedStartDate = {};
      if (filters.startDate) baseWhere.plannedStartDate.gte = new Date(filters.startDate);
      if (filters.endDate) baseWhere.plannedStartDate.lte = new Date(filters.endDate);
    }

    const result = await applyGridQuery<any>(this.prisma.workOrder, filters, {
      baseWhere,
      allowedFields: ManufacturingService.WORK_ORDER_ALLOWED_FIELDS,
      defaultOrderBy: { plannedStartDate: 'asc' },
      include: { operations: true, materialIssues: true, completions: true },
    });

    const productIds = [...new Set(result.items.map((wo: any) => wo.productId))];
    ...
    return {
      ...result,
      items: result.items.map((wo: any) => this.mapWorkOrderResponse(wo, productMap.get(wo.productId))),
    };
  }
  ...
  private static readonly QUALITY_INSPECTION_ALLOWED_FIELDS: AllowedFields = {
  ...
  async getQualityInspections(tenantId: string, params: {
    ...
    const { status, inspectionType, workOrderId, purchaseOrderId, productId } = params;
    return applyGridQuery<any>(this.prisma.qualityInspection, params, {
      baseWhere: {
        tenantId,
        ...(status && { status }),
      ...
      allowedFields: ManufacturingService.QUALITY_INSPECTION_ALLOWED_FIELDS,
      defaultOrderBy: { inspectionDate: 'desc' },
      include: {
  ...
  private static readonly UOM_ALLOWED_FIELDS: AllowedFields = {
    code: 'string',
    name: 'string',
    symbol: 'string',
    category: 'enum',
  ...
  async getUoms(tenantId: string, params: {
    ...
    return applyGridQuery<any>(this.prisma.unitOfMeasure, params, {
      baseWhere: {
        tenantId,
        ...(category && { category }),
        ...(isActive !== undefined && { isActive }),
      ...
      allowedFields: ManufacturingService.UOM_ALLOWED_FIELDS,
  ...
  private static readonly UOM_CONVERSION_ALLOWED_FIELDS: AllowedFields = {
    fromUom: 'string',
    toUom: 'string',
    factor: 'number',
    isActive: 'boolean',
  ...
  async getUomConversions(tenantId: string, params: {
    ...
    const { productId, fromUom, toUom, isActive } = params;
    return applyGridQuery<any>(this.prisma.unitOfMeasureConversion, params, {
      baseWhere: {
        tenantId,
        ...(productId && { productId }),
        ...(fromUom && { fromUom }),
      ...
      allowedFields: ManufacturingService.UOM_CONVERSION_ALLOWED_FIELDS,
      defaultOrderBy: { createdAt: 'desc' },
      include: {
  ...
  private static readonly CAPACITY_PLAN_ALLOWED_FIELDS = {
    name: 'string',
    description: 'string',
    planType: 'enum',
    status: 'enum',
  ...
  async getCapacityPlans(tenantId: string, params: {
    ...
    const { status, planType } = params;
    return applyGridQuery<any>(this.prisma.capacityPlan, params, {
      baseWhere: {
        tenantId,
        ...(status && { status }),
        ...(planType && { planType }),
      },
      allowedFields: ManufacturingService.CAPACITY_PLAN_ALLOWED_FIELDS,
      defaultOrderBy: { createdAt: 'desc' },
  ...
  private static readonly PURCHASE_CONTRACT_ALLOWED_FIELDS = {
  ...
  async getPurchaseContracts(tenantId: string, params: {
    ...
    const { supplierId, status, contractType } = params;
    return applyGridQuery<any>(this.prisma.purchaseContract, params, {
      baseWhere: {
        tenantId,
      ...
      allowedFields: ManufacturingService.PURCHASE_CONTRACT_ALLOWED_FIELDS,
      defaultOrderBy: { createdAt: 'desc' },
      include: { lines: true },
    });
  }
  ...
  private static readonly PRODUCT_COSTING_ALLOWED_FIELDS = {
    costType: 'enum',
    materialCost: 'number',
    laborCost: 'number',
    overheadCost: 'number',
  ...
  async getProductCostings(tenantId: string, params: {
    ...
    const { productId, locationId, costType } = params;
    return applyGridQuery<any>(this.prisma.productCosting, params, {
      baseWhere: {
        tenantId,
        ...(productId && { productId }),
        ...(locationId && { locationId }),
      ...
      allowedFields: ManufacturingService.PRODUCT_COSTING_ALLOWED_FIELDS,
      defaultOrderBy: { effectiveFrom: 'desc' },
    });
  }
  ...
  private static readonly BATCH_ALLOWED_FIELDS: AllowedFields = {
    batchNumber: 'string',
    productId: 'string',
    locationId: 'string',
    status: 'enum',
  ...
  async getBatches(tenantId: string, params?: {
    ...
    const { status, productId, locationId, expiringBefore } = params ?? {};

    const result = await applyGridQuery<any>(this.prisma.batch, params, {
      baseWhere: {
        tenantId,
        ...(status && { status }),
        ...(productId && { productId }),
      ...
      allowedFields: ManufacturingService.BATCH_ALLOWED_FIELDS,
      defaultOrderBy: { createdAt: 'desc' },
    });

    // Enrich with product and location names (single round-trip each)
    const productIds = [...new Set(result.items.map((i: any) => i.productId))];
    const locationIds = [...new Set(result.items.map((i: any) => i.locationId))];
    const [products, locations] = await Promise.all([
    ...
    return {
      ...result,
      items: result.items.map((b: any) => ({ ...b, product: prodMap.get(b.productId) ?? null, location: locMap.get(b.locationId) ?? null })),
    };
  }
  ...
  private static readonly INVENTORY_RESERVATION_ALLOWED_FIELDS = {
    referenceType: 'enum',
    status: 'enum',
    reservedQty: 'number',
    fulfilledQty: 'number',
  ...
  async getInventoryReservations(tenantId: string, params: {
    ...
    const { productId, referenceType, referenceId, status } = params;
    const result = await applyGridQuery<any>(this.prisma.inventoryReservation, params, {
      baseWhere: {
        tenantId,
        ...(productId && { productId }),
      ...
      allowedFields: ManufacturingService.INVENTORY_RESERVATION_ALLOWED_FIELDS,
      defaultOrderBy: { createdAt: 'desc' },
    });
  ...
  private static readonly INVENTORY_HOLD_ALLOWED_FIELDS = {
    holdReason: 'enum',
    status: 'enum',
    heldQty: 'number',
    releasedQty: 'number',
  ...
  async getInventoryHolds(tenantId: string, params: {
    ...
    const { productId, status, holdReason } = params;
    const result = await applyGridQuery<any>(this.prisma.inventoryHold, params, {
      baseWhere: {
        tenantId,
        ...(productId && { productId }),
        ...(status && { status }),
      ...
      allowedFields: ManufacturingService.INVENTORY_HOLD_ALLOWED_FIELDS,
      defaultOrderBy: { createdAt: 'desc' },
    });
  ...
  private static readonly PRODUCT_CATEGORY_ALLOWED_FIELDS: AllowedFields = {
    code: 'string',
    name: 'string',
    description: 'string',
    isActive: 'boolean',
  ...
  async getProductCategories(tenantId: string, params?: {
    ...
    if (params?.parentId !== undefined) baseWhere.parentId = params.parentId || null;
    if (params?.search) {
      baseWhere.OR = [
    ...
    const result = await applyGridQuery<any>(this.prisma.productCategory, params, {
      baseWhere,
      allowedFields: ManufacturingService.PRODUCT_CATEGORY_ALLOWED_FIELDS,
    ...
    // Frontend ProductCategoryService expects `data` key (not `items`)
  ...
  private static readonly PRODUCTION_LINE_ALLOWED_FIELDS = {
    code: 'string',
    name: 'string',
    description: 'string',
    status: 'enum',
  ...
  async getProductionLines(tenantId: string, params?: {
    status?: string;
    page?: number;
    pageSize?: number;
    sortBy?: string;
    ...
    return applyGridQuery<any>(this.prisma.productionLine, params, {
      baseWhere: { tenantId, ...(params?.status ? { status: params.status as any } : {}) },
      allowedFields: ManufacturingService.PRODUCTION_LINE_ALLOWED_FIELDS,
      defaultOrderBy: { code: 'asc' },
  ...
  private static readonly DOWNTIME_RECORD_ALLOWED_FIELDS = {
  ...
  async getDowntimeRecords(tenantId: string, params?: {
    ...
    const baseWhere: any = { tenantId };
    if (params?.productionLineId) baseWhere.productionLineId = params.productionLineId;
    if (params?.startDate || params?.endDate) {
      baseWhere.startTime = {};
      if (params?.startDate) baseWhere.startTime.gte = new Date(params.startDate);
      if (params?.endDate) baseWhere.startTime.lte = new Date(params.endDate);
    }
    return applyGridQuery<any>(this.prisma.downtimeRecord, params, {
      baseWhere,
      allowedFields: ManufacturingService.DOWNTIME_RECORD_ALLOWED_FIELDS,
