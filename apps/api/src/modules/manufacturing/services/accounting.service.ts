...
export class AccountingService {
  ...
  private static readonly GL_ACCOUNT_ALLOWED_FIELDS = {
    accountNumber: 'string',
    name: 'string',
    description: 'string',
    accountType: 'enum',
  ...
  async getGLAccounts(
    ...
    return applyGridQuery<any>(this.prisma.gLAccount, filters, {
      baseWhere: {
        tenantId,
        ...(filters?.accountType && { accountType: filters.accountType }),
        ...(filters?.isActive !== undefined && { isActive: filters.isActive }),
      },
      allowedFields: AccountingService.GL_ACCOUNT_ALLOWED_FIELDS,
      defaultOrderBy: { accountNumber: 'asc' },
      defaultPageSize: 100,
  ...
  private static readonly JOURNAL_ENTRY_ALLOWED_FIELDS = {
    entryNumber: 'string',
    description: 'string',
    referenceType: 'enum',
    referenceId: 'string',
  ...
  async getJournalEntries(
    ...
    // Honour legacy skip/take pagination if supplied; otherwise use page/pageSize from filters
    const effectiveParams = pagination?.skip !== undefined || pagination?.take !== undefined
      ? {
    ...
    const result = await applyGridQuery<any>(this.prisma.journalEntry, effectiveParams, {
      baseWhere,
      allowedFields: AccountingService.JOURNAL_ENTRY_ALLOWED_FIELDS,
      defaultOrderBy: { createdAt: 'desc' },
      defaultPageSize: 50,
      include: { lines: { include: { glAccount: true } } },
    });

    // Maintain legacy `{ entries, total }` shape for existing callers
    return { entries: result.items, total: result.total, page: result.page, pageSize: result.pageSize, totalPages: result.totalPages };
  }
