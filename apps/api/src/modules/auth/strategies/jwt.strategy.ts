...
export class JwtStrategy extends PassportStrategy(Strategy) {
  ...
  async validate(payload: JwtPayload) {
    ...
    if (user.status !== 'ACTIVE') {
      throw new UnauthorizedException({ code: 'TOKEN_INVALID', message: 'User is not active' });
    }

    // Verify tenant still matches and is active (SUPER_ADMIN can access any tenant state)
    if (user.tenantId !== payload.tenantId) {
      throw new UnauthorizedException({ code: 'TOKEN_INVALID', message: 'Invalid tenant' });
    }

    if (!payload.sessionId) {
      throw new UnauthorizedException({ code: 'SESSION_REPLACED', message: 'Session is no longer active' });
    }
    ...
    if (user.role !== 'SUPER_ADMIN') {
      ...
      if (!tenant || (tenant.status !== 'ACTIVE' && tenant.status !== 'TRIAL')) {
        throw new UnauthorizedException({ code: 'TOKEN_INVALID', message: 'Tenant is not active' });
      }
    }
