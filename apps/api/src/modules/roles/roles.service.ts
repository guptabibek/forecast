...
import { AI_REPORTING_ROLE_PERMISSION_ADDITIONS, ALL_PERMISSION_KEYS, SYSTEM_ROLE_TEMPLATES } from './rbac.constants';
...
export class RolesService {
  ...
  async ensureDefaultAdminRole(
    ...
    const existing = await client.tenantRole.findFirst({
      ...
      select: { id: true, name: true, slug: true, permissions: true, moduleAccess: true },
    });

    if (existing) {
      return this.ensureSystemRoleAdditions(existing, client);
    }
  ...
  async seedSystemRoles(tenantId: string) {
    ...
    for (const tmpl of SYSTEM_ROLE_TEMPLATES) {
      const existing = existingBySlug.get(tmpl.slug);
      if (existing) {
        await this.ensureSystemRoleAdditions(existing, this.prisma);
        continue;
      }
  ...
  private async ensureSystemRoleAdditions(
    role: { id: string; name: string; slug: string; permissions: unknown; moduleAccess: unknown },
    client: Prisma.TransactionClient | PrismaService,
  ) {
    const additions = AI_REPORTING_ROLE_PERMISSION_ADDITIONS[role.slug] ?? [];
    const currentPermissions = this.asStringArray(role.permissions);
    const nextPermissions = this.mergeUnique(currentPermissions, additions);
    const currentModuleAccess = this.asRecord(role.moduleAccess);
    const nextModuleAccess = additions.length > 0
      ? { ...currentModuleAccess, reports: true }
      : currentModuleAccess;
    ...
    if (JSON.stringify(nextModuleAccess) !== JSON.stringify(currentModuleAccess)) {
      data.moduleAccess = nextModuleAccess as Prisma.InputJsonValue;
    }

    if (Object.keys(data).length > 0) {
      await client.tenantRole.update({
        where: { id: role.id },
        data,
      });
    }
  ...
  private asRecord(value: unknown): Record<string, boolean> {
    if (!value || typeof value !== 'object' || Array.isArray(value)) return {};
    return Object.entries(value as Record<string, unknown>).reduce<Record<string, boolean>>((acc, [key, item]) => {
