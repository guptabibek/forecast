...
const NEAR_EXPIRY_COLUMNS: AllowedSqlColumns = {
  ...
  remaining_days: { expression: '(b.expiry_date::date - CURRENT_DATE)', type: 'number' },
...
const EXPIRED_STOCK_COLUMNS: AllowedSqlColumns = {
  ...
  location_name: { expression: 'l.name', type: 'string' },
  expiry_date: { expression: 'b.expiry_date', type: 'date' },
  days_expired: { expression: '(CURRENT_DATE - b.expiry_date::date)', type: 'number' },
  quantity: { expression: 'b.quantity', type: 'number' },
  cost_per_unit: { expression: 'COALESCE(b.cost_per_unit, 0)', type: 'number' },
  expired_value: { expression: '(COALESCE(b.quantity, 0) * COALESCE(b.cost_per_unit, 0))', type: 'number' },
  batch_status: { expression: 'b.status', type: 'enum' },
};

const FEFO_COLUMNS: AllowedSqlColumns = {
  sku: { expression: 'p.code', type: 'string' },
  product_name: { expression: 'p.name', type: 'string' },
  category: { expression: 'p.category', type: 'string' },
  batch_number: { expression: 'b.batch_number', type: 'string' },
...
export class ExpiryReportsService {
  ...
  async getNearExpiry(
    ...
    const filterConds = buildPharmaFilterSql(columnFilters, NEAR_EXPIRY_COLUMNS);
    ...
    const orderBy = buildPharmaOrderBySql(
      filters.sortBy, filters.sortDir, NEAR_EXPIRY_COLUMNS,
      Prisma.sql`(b.expiry_date::date - CURRENT_DATE) ASC, (COALESCE(b.quantity, 0) * COALESCE(b.cost_per_unit, 0)) DESC`,
    );
  ...
  async getExpiredStock(
    ...
    const columnFilters = parsePharmaFilters(filters.filters);
    const filterConds = buildPharmaFilterSql(columnFilters, EXPIRED_STOCK_COLUMNS);
    ...
    const where = allConds.length
      ...
      : baseCond;
    const orderBy = buildPharmaOrderBySql(
      filters.sortBy, filters.sortDir, EXPIRED_STOCK_COLUMNS,
      Prisma.sql`(CURRENT_DATE - b.expiry_date::date) DESC, (COALESCE(b.quantity, 0) * COALESCE(b.cost_per_unit, 0)) DESC`,
    );
  ...
  async getFEFOPickingSequence(
    ...
    const columnFilters = parsePharmaFilters(filters.filters);
    const filterConds = buildPharmaFilterSql(columnFilters, FEFO_COLUMNS);
    ...
    const where = allConds.length
      ...
      : baseCond;
    const orderBy = buildPharmaOrderBySql(
      filters.sortBy, filters.sortDir, FEFO_COLUMNS,
      Prisma.sql`p.code, l.code, picking_sequence`,
    );
