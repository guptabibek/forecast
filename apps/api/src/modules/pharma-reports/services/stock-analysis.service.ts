...
const DEAD_SLOW_COLUMNS: AllowedSqlColumns = {
  ...
  last_sale_date: { expression: 'ls.last_sale_date', type: 'date' },
};

const TURNOVER_COLUMNS: AllowedSqlColumns = {
  sku: { expression: 'p.code', type: 'string' },
  product_name: { expression: 'p.name', type: 'string' },
  category: { expression: 'p.category', type: 'string' },
  location_code: { expression: 'l.code', type: 'string' },
  cogs: { expression: 'COALESCE(c.cogs_value, 0)', type: 'number' },
  avg_inventory: { expression: 'COALESCE(il.inventory_value, 0)', type: 'number' },
};

const ABC_COLUMNS: AllowedSqlColumns = {
  sku: { expression: 'r.sku', type: 'string' },
  product_name: { expression: 'r.product_name', type: 'string' },
  consumption_value: { expression: 'r.consumption_value', type: 'number' },
  consumption_qty: { expression: 'r.consumption_qty', type: 'number' },
  on_hand_qty: { expression: 'r.on_hand_qty', type: 'number' },
  inventory_value: { expression: 'r.inventory_value', type: 'number' },
  pct_of_total: { expression: 'r.pct_of_total', type: 'number' },
};

const XYZ_COLUMNS: AllowedSqlColumns = {
  sku: { expression: 'p.code', type: 'string' },
  product_name: { expression: 'p.name', type: 'string' },
  category: { expression: 'p.category', type: 'string' },
  avg_monthly_demand: { expression: 'COALESCE(ps.avg_monthly_demand, 0)', type: 'number' },
  stddev_monthly_demand: { expression: 'COALESCE(ps.stddev_monthly_demand, 0)', type: 'number' },
  months_analyzed: { expression: 'COALESCE(ps.months_analyzed, 0)', type: 'number' },
};
...
export class StockAnalysisService {
  ...
  async getDeadSlowStock(
    ...
    const filterConds = buildPharmaFilterSql(columnFilters, DEAD_SLOW_COLUMNS);
    ...
    const orderBy = buildPharmaOrderBySql(
      filters.sortBy, filters.sortDir, DEAD_SLOW_COLUMNS,
      Prisma.sql`COALESCE(il.inventory_value, 0) DESC`,
    );
  ...
  async getABCAnalysis(
    ...
    const columnFilters = parsePharmaFilters(filters.filters);
    const abcFilterConds = buildPharmaFilterSql(columnFilters, ABC_COLUMNS);
    const abcFilterClause = abcFilterConds.length
      ? Prisma.sql`WHERE ${Prisma.join(abcFilterConds, ' AND ')}`
      : Prisma.empty;
    const abcOrderBy = buildPharmaOrderBySql(
      filters.sortBy, filters.sortDir, ABC_COLUMNS,
      Prisma.sql`r.consumption_value DESC`,
    );
  ...
  async getXYZAnalysis(
    ...
    const columnFilters = parsePharmaFilters(filters.filters);
    const xyzFilterConds = buildPharmaFilterSql(columnFilters, XYZ_COLUMNS);
    const allXyzConds = [...extraConds, ...xyzFilterConds];
    const prodFilter = allXyzConds.length
      ? Prisma.sql`AND ${Prisma.join(allXyzConds, ' AND ')}`
      : Prisma.empty;
    const xyzOrderBy = buildPharmaOrderBySql(
      filters.sortBy, filters.sortDir, XYZ_COLUMNS,
      Prisma.sql`COALESCE(ps.stddev_monthly_demand, 0) / NULLIF(COALESCE(ps.avg_monthly_demand, 0), 0) ASC NULLS LAST`,
    );
  ...
  async getInventoryTurnover(
    ...
    const columnFilters = parsePharmaFilters(filters.filters);
    const filterConds = buildPharmaFilterSql(columnFilters, TURNOVER_COLUMNS);
    ...
    const where = allConds.length
      ...
      : baseCond;
    const orderBy = buildPharmaOrderBySql(
      filters.sortBy, filters.sortDir, TURNOVER_COLUMNS,
      Prisma.sql`CASE WHEN COALESCE(il.inventory_value, 0) > 0 THEN (COALESCE(c.cogs_value, 0) / il.inventory_value) ELSE NULL END ASC NULLS LAST`,
    );
