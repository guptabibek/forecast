...
20. Outstanding / receivable / payable / "amount due" questions (dataset party_outstanding) report a point-in-time BALANCE — every open bill as of today, regardless of invoice date. Do NOT attach a time range to these queries: a period would silently shrink the balance to only the bills raised in that window (e.g. "total outstanding for customer X" collapsing to this month's bills). Set time preset "unspecified" (or omit it). Use a date range on party_outstanding ONLY when the user explicitly asks to bound it by invoice/bill date ("outstanding on bills raised in May"); state that interpretation in assumptions[].`;
}
