...
const AiReporting = lazy(() => import('@pages/reports/AiReporting'));
...
function RoleAwareRoute({
  ...
  // Forecast planner/viewer roles get read-only access to specific planning-relevant
  // manufacturing surfaces (PO, PI, Inventory, Suppliers, Batches) — bypass the
  // restrictManufacturing guard for those paths.
  const planningPathException =
    restrictManufacturing &&
    isManufacturingBlockedRole(role) &&
    isPlanningVisibleManufacturingPath(location.pathname);

  const blocked =
    (restrictForecastViewer && isForecastViewerRole(role) && !planningPathException) ||
    (restrictManufacturing && isManufacturingBlockedRole(role) && !planningPathException);
  ...
  return (
    ...
    <Routes>
      ...
      >
        ...
        <Route path="/settings/users" element={<SuperAdminRoute><Users /></SuperAdminRoute>} />
        <Route path="/settings/roles" element={<SuperAdminRoute><Roles /></SuperAdminRoute>} />
