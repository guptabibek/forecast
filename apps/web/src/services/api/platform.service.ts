...
export const platformService = {
  ...
  async createTenantUser(
    ...
    const { data } = await apiClient.post<{ data: TenantUser }>(
      `/platform/tenants/${tenantId}/users`,
      payload,
    );
    return data.data;
  ...
  async updateTenantUser(
    ...
    const { data } = await apiClient.patch<{ data: TenantUser }>(
  ...
  async deactivateTenantUser(tenantId: string, userId: string): Promise<void> {
    await apiClient.delete(`/platform/tenants/${tenantId}/users/${userId}`);
  },
