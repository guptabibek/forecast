...
export interface ManufacturingGlAccountsResponse {
  items: ManufacturingGlAccount[];
  total: number;
  page: number;
  pageSize: number;
  totalPages: number;
...
export const manufacturingService = {
  ...
  async getGLAccounts(filters?: ManufacturingGlAccountFilters): Promise<ManufacturingGlAccountsResponse> {
    const { data } = await apiClient.get<ManufacturingGlAccountsResponse>('/manufacturing/gl-accounts', {
      params: filters,
    });
    return data;
  },
