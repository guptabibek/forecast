...
type BrandingSettings = TenantSettings | PublicTenantSettings;
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  const { data: authenticatedSettings, isLoading: isAuthenticatedSettingsLoading } = useQuery<TenantSettings>({
  ...
  const { data: publicSettings, isLoading: isPublicSettingsLoading } = useQuery<PublicTenantSettings>({
    queryKey: settingsKeys.publicTenant(publicWorkspaceHost),
    queryFn: () => settingsService.getPublicSettings(publicWorkspaceHost),
    staleTime: 2 * 60 * 1000,
    retry: 1,
    enabled: !isAuthenticated && Boolean(publicWorkspaceHost),
  });

  const settings = isAuthenticated
    ? authenticatedSettings ?? publicSettings ?? null
    : publicSettings ?? null;
  ...
  const refresh = useCallback(() => {
    if (isAuthenticated) {
      ...
      queryClient.invalidateQueries({ queryKey: settingsKeys.publicTenant(publicWorkspaceHost) });
    }
  }, [isAuthenticated, publicWorkspaceHost, queryClient]);
