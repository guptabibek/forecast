import type { Dimension, DimensionType } from '@/types';
...
const dimensionTypes: { value: DimensionType; label: string; icon: React.ReactNode }[] = [
  { value: 'location', label: 'Locations', icon: <MapPinIcon className="w-5 h-5" /> },
  { value: 'customer', label: 'Customers', icon: <UserGroupIcon className="w-5 h-5" /> },
  { value: 'account', label: 'Accounts', icon: <CurrencyDollarIcon className="w-5 h-5" /> },
];
...
export default function Dimensions() {
  ...
  const onSubmit = (data: DimensionFormData) => {
    ...
    if (selectedParent?.id) {
      cleanedData.parentId = selectedParent.id;
    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-secondary-500 mt-1">
            Manage products, locations, customers, and accounts
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="p-12 text-center">
            <div className="mx-auto w-12 h-12 rounded-full bg-secondary-100 dark:bg-secondary-800 flex items-center justify-center mb-4">
              {dimensionTypes.find((t) => t.value === activeType)?.icon}
            </div>
            <h3 className="text-lg font-medium">No {activeType}s found</h3>
            <p className="text-secondary-500 mt-1">
                  ...
                  <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        {...register('name')}
                        className="input w-full"
                        placeholder={`${activeType.charAt(0).toUpperCase() + activeType.slice(1)} name`}
                      />
                      {errors.name && (
                        <p className="text-sm text-red-500 mt-1">
                          {errors.name.message}
