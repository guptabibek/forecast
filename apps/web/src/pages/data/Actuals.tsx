  ...
  const columns = [
    ...
    columnHelper.accessor((row) => getValue(row), {
      ...
      cell: (info) => {
        const value = info.getValue();
        return value != null ? (
          <span className="font-mono text-right block">{formatInr(value)}</span>
        ) : (
          <span className="text-secondary-400">-</span>
        );
      },
    ...
    columnHelper.accessor('quantity', {
      ...
      cell: (info) => {
        const value = info.getValue();
        return value != null ? (
          <span className="font-mono text-right block">{formatIndianNumber(value)}</span>
        ) : (
          <span className="text-secondary-400">-</span>
        );
      },
  ...
  return (
    <div className="space-y-4 lg:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-xl lg:text-2xl font-bold">Actuals Data</h1>
      ...
      {summary && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="card p-4">
            ...
            <p className="text-xl lg:text-2xl font-bold">{formatIndianNumber(summary.totalRecords)}</p>
          </div>
          <div className="card p-4">
      ...
      <div className="card">
        ...
        <div className="p-4 border-t border-secondary-200 dark:border-secondary-700 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-sm text-secondary-500">
            Showing {actuals.length > 0 ? ((page - 1) * pageSize) + 1 : 0} to {Math.min(page * pageSize, totalRecords)} of {formatIndianNumber(totalRecords)} records
          </p>
          <div className="flex items-center gap-2">
            <button
              onClick={() => setPage(1)}
