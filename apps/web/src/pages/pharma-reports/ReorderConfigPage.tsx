...
export default function ReorderConfigPage() {
  ...
  const downloadTemplate = async () => {
    ...
    try {
      ...
      // Fall back to a single example row if the tenant has no stock yet.
      const body = lines.length
        ? lines.join('\n')
        : ['SKU001', 'MAIN', '100', '25', '600', '12', '', '', '', '7', 'A', 'Example product', 'Main store'].map(csvEscape).join(',');
      triggerCsvDownload(`${headers.join(',')}\n${body}\n`, 'reorder-config-template.csv');
    } catch {
    ...
    <div className="space-y-4 lg:space-y-6">
      <div className="flex flex-wrap justify-between items-start gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" size="sm" isLoading={templateBusy} onClick={() => void downloadTemplate()}>Download template</Button>
          <Button
            variant="outline"
            size="sm"
          ...
          <Button variant="primary" size="sm" onClick={openAdd}>Add policy</Button>
          <input
            ref={csvInputRef}
            type="file"
            accept=".csv,text/csv"
      ...
      {list.isError && <QueryErrorBanner error={list.error} />}
