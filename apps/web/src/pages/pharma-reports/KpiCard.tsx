...
export default function KpiCard({ title, value, subtitle, icon, color = 'blue' }: KpiCardProps) {
  ...
  return (
    <div className={clsx('rounded-xl border p-3 lg:p-5 transition-shadow hover:shadow-md', c.bg, c.border)}>
      <div className="flex items-start justify-between gap-2">
        <div className="space-y-0.5 lg:space-y-1 min-w-0">
          <p className="text-xs lg:text-sm font-medium text-gray-500 dark:text-secondary-400 truncate">{title}</p>
          <p className="text-base lg:text-xl font-bold tracking-tight text-gray-900 dark:text-white truncate">{value}</p>
          {subtitle && <p className="text-[10px] lg:text-xs text-gray-500 dark:text-secondary-400 line-clamp-2">{subtitle}</p>}
        </div>
        <div className={clsx('rounded-lg bg-white/80 p-2 lg:p-2.5 dark:bg-secondary-900/70 flex-shrink-0', c.icon)}>
          {icon}
        </div>
      </div>
    </div>
