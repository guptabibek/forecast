  ...
  return (
    <div className="space-y-4 lg:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-xl lg:text-2xl font-bold">Forecasts</h1>
                    ...
                    {enhancedAccuracyData.byModel.map((model, index) => (
                      ...
                      >
                        ...
                        <td className="py-2 px-3 text-center text-secondary-600">
                          {model.mae !== null ? formatInr(model.mae) : 'N/A'}
                        </td>
                        <td className="py-2 px-3 text-center">
                          {model.bias !== null ? (
                      ...
                      {backtestData.results.map((result, index) => (
                        ...
                        >
                          ...
                          <td className="py-2 px-2 text-center text-secondary-600">
                            {result.metrics.mae !== null ? formatInr(result.metrics.mae) : 'N/A'}
                          </td>
                          <td className="py-2 px-2 text-center">
                  ...
                  <div className="mt-4">
                    ...
                    <ResponsiveContainer width="100%" height={200}>
                      <LineChart data={versionComparisonData.periods}>
                        ...
                        <YAxis stroke="#94a3b8" fontSize={10} tickFormatter={(v: number) => formatInr(v, 0)} />
                        <Tooltip
              ...
              >
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {(() => {
                        const items: DimensionBreakdownItem[] = Array.isArray(dimensionData)
                          ? dimensionData
                          : (dimensionData as { items?: DimensionBreakdownItem[] } | undefined)?.items || [];
                        const grandTotal = items.reduce((sum, d) => sum + (d.totalAmount ?? d.total ?? 0), 0);
                        return items.slice(0, 20).map((dim, idx) => (
                          <tr key={dim.id || dim.dimensionId || idx} className="border-b border-secondary-100 dark:border-secondary-800">
                            ...
                            <td className="py-2 px-3 text-right font-medium">{formatInr(dim.totalAmount ?? dim.total ?? 0)}</td>
