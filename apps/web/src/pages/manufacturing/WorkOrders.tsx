...
export default function WorkOrdersPage() {
  ...
  const { data: workOrders, isLoading, isError, error, refetch } = useQuery({
    queryKey: ['manufacturing', 'work-orders', grid.queryKey],
    queryFn: () => workOrderService.getAll(grid.queryParams),
  ...
  const items: WorkOrder[] = Array.isArray((workOrders as any)?.items) ? (workOrders as any).items : Array.isArray(workOrders) ? workOrders : [];
  const total = (workOrders as any)?.total ?? items.length;
  ...
  const columns: Column<WorkOrder>[] = [
    {
      ...
      sortable: true, filterType: 'text', filterField: 'woNumber',
    },
    {
      key: 'product',
      header: 'Product',
    ...
    },
    {
      ...
      align: 'right', sortable: true, filterType: 'number', filterField: 'quantity',
    },
    {
      key: 'status',
      header: 'Status',
    ...
    },
    {
      key: 'priority',
      header: 'Priority',
      accessor: (row) => priorityLabels[row.priority] || String(row.priority),
      align: 'right',
      filterType: 'number', filterField: 'priority',
    },
    {
      key: 'scheduledStart',
      header: 'Start',
      accessor: (row) => safeFormat(row.scheduledStart, 'MMM dd, yyyy'),
      sortable: true, filterType: 'date', filterField: 'scheduledStart',
    },
    {
      key: 'scheduledEnd',
      header: 'End',
      accessor: (row) => safeFormat(row.scheduledEnd, 'MMM dd, yyyy'),
      sortable: true, filterType: 'date', filterField: 'scheduledEnd',
    },
    {
      key: 'actions',
      header: 'Actions',
  ...
  return (
    <div className="space-y-4 lg:space-y-6">
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-xl lg:text-2xl font-bold">Work Orders</h1>
          <p className="text-xs lg:text-sm text-secondary-500 mt-1">Manage production work orders and shop floor execution</p>
        </div>
        {canMutate && (
      ...
      {/* Status Summary Cards (current page) */}
          ...
          return (
            ...
            >
              <div className="p-4 text-center">
                ...
                <div className="text-xs lg:text-sm text-secondary-500">{status.replace('_', ' ')}</div>
              </div>
            </div>
          );
        })}
      ...
      <Card>
        <CardHeader
          title="Work Orders"
          description="All work orders. Filter per column · sort by clicking header · server-side."
        />
        <DataTable
          data={items}
          columns={columns}
      ...
      >
        {woDetail && (
          <div className="space-y-4 lg:space-y-6">
            <DetailPopupActions
              title="Work Order"
              documentNumber={selectedWO?.woNumber}
              fields={[
                { label: 'WO Number', value: selectedWO?.woNumber },
                { label: 'Product', value: woDetail.product?.name },
                { label: 'Status', value: woDetail.status },
                { label: 'Quantity', value: woDetail.quantity },
                { label: 'Completed', value: woDetail.completedQuantity },
                { label: 'Priority', value: selectedWO?.priority ? priorityLabels[selectedWO.priority] || String(selectedWO.priority) : undefined },
              ]}
              tables={woDetail.operations?.length ? [{
                title: 'Operations',
                columns: [
                  ...
                  { key: 'plannedTime', header: 'Planned Time', align: 'right' },
                  { key: 'actualTime', header: 'Actual Time', align: 'right' },
                ],
                rows: woDetail.operations.map((op: WorkOrderOperation) => ({
                  sequence: op.sequence,
                  operationName: op.operationName,
                  workCenter: op.workCenter?.name || '—',
                  status: op.status,
                  plannedTime: `${op.plannedSetupTime + op.plannedRunTime}h`,
                  actualTime: `${(op.actualSetupTime || 0) + (op.actualRunTime || 0)}h`,
                })),
              }] : []}
              totals={[
                { label: 'Total Quantity', value: String(woDetail.quantity) },
                { label: 'Completed', value: String(woDetail.completedQuantity) },
              ]}
            />
