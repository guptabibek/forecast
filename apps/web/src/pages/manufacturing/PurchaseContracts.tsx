...
export default function PurchaseContractsPage() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: ['manufacturing', 'purchase-contracts', grid.queryKey],
    queryFn: () => purchaseContractService.getAll(grid.queryParams),
  ...
  const columns: Column<PurchaseContract>[] = [
    { key: 'contractNumber', header: 'Contract #', accessor: 'contractNumber', sortable: true, filterType: 'text', filterField: 'contractNumber' },
    { key: 'supplier', header: 'Supplier', accessor: (r) => r.supplier?.name || r.supplierId },
    {
      ...
      filterType: 'select', filterField: 'contractType',
      filterOptions: [
    ...
    { key: 'startDate', header: 'Start Date', accessor: (r) => safeFormat(r.startDate, 'MMM dd, yyyy'), sortable: true, filterType: 'date', filterField: 'startDate' },
    { key: 'endDate', header: 'End Date', accessor: (r) => safeFormat(r.endDate, 'MMM dd, yyyy'), sortable: true, filterType: 'date', filterField: 'endDate' },
    { key: 'totalValue', header: 'Total Value', accessor: (r) => r.totalValue ? formatInr(r.totalValue) : '—', align: 'right', filterType: 'number', filterField: 'totalValue' },
    { key: 'currency', header: 'Currency', accessor: 'currency', filterType: 'text', filterField: 'currency' },
    {
  ...
  return (
    <div className="space-y-4 lg:space-y-6">
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-xl lg:text-2xl font-bold">Purchase Contracts</h1>
        ...
        {selected && (
          <div className="space-y-4 lg:space-y-6">
            <DetailPopupActions
              ...
              documentNumber={selected.contractNumber}
              fields={[
                { label: 'Contract #', value: selected.contractNumber },
                { label: 'Supplier', value: (contractDetail as any)?.supplier?.name || selected.supplier?.name || selected.supplierId },
                { label: 'Type', value: selected.contractType },
                { label: 'Status', value: selected.status },
                { label: 'Currency', value: selected.currency },
                { label: 'Start Date', value: safeFormat(selected.startDate, 'MMM dd, yyyy') },
                { label: 'End Date', value: safeFormat(selected.endDate, 'MMM dd, yyyy') },
                { label: 'Total Value', value: selected.totalValue ? formatInr(selected.totalValue) : '—' },
                { label: 'Consumed', value: formatInr(selected.consumedValue ?? 0) },
              ]}
              tables={linesData.length ? [{
                title: 'Contract Lines',
                columns: [
                  { key: 'product', header: 'Product' },
                  { key: 'agreedPrice', header: 'Agreed Price', align: 'right' },
                  { key: 'agreedQty', header: 'Agreed Qty', align: 'right' },
                  { key: 'consumedQty', header: 'Consumed Qty', align: 'right' },
                  { key: 'minOrderQty', header: 'Min Order Qty', align: 'right' },
                  { key: 'leadTimeDays', header: 'Lead Time' },
                  { key: 'uom', header: 'UOM' },
                ],
                rows: linesData.map((l) => ({
                  product: l.product?.name || l.productId,
                  agreedPrice: formatInr(l.agreedPrice),
                  agreedQty: l.agreedQty?.toLocaleString() ?? '—',
                  consumedQty: String(l.consumedQty ?? 0),
                  minOrderQty: String(l.minOrderQty ?? '—'),
                  leadTimeDays: l.leadTimeDays ? `${l.leadTimeDays}d` : '—',
                  uom: l.uom || '—',
                })),
              }] : []}
              totals={[
                { label: 'Total Value', value: selected.totalValue ? formatInr(selected.totalValue) : '—' },
                { label: 'Consumed Value', value: formatInr(selected.consumedValue ?? 0) },
              ]}
            />
            ...
            <div className="grid grid-cols-4 gap-4 text-sm">
              ...
              <div><span className="font-medium text-gray-500">Total Value:</span> {selected.totalValue ? formatInr(selected.totalValue) : '—'}</div>
              <div><span className="font-medium text-gray-500">Consumed:</span> {formatInr(selected.consumedValue ?? 0)}</div>
            </div>
                  ...
                  <tbody>
                    ...
                    {linesData.map((l) => (
                      <tr key={l.id} className="border-t">
                        ...
                        <td className="px-3 py-2 text-right">{formatInr(l.agreedPrice)}</td>
