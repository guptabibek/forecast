...
export default function ForecastAccuracyPage() {
  ...
  const grid = useGridState({ initialSortBy: 'periodDate', initialSortOrder: 'desc' });

  // Queries
  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: ['manufacturing', 'forecast-accuracy', grid.queryKey],
    queryFn: () => forecastAccuracyService.getAll(grid.queryParams),
  ...
  const columns: Column<ForecastAccuracyMetric>[] = [
    ...
    { key: 'periodDate', header: 'Period', accessor: (r) => safeFormat(r.periodDate, 'MMM yyyy'), sortable: true, filterType: 'date', filterField: 'periodDate' },
    { key: 'forecastQty', header: 'Forecast Value', accessor: (r) => r.forecastQty?.toLocaleString(), align: 'right', filterType: 'number', filterField: 'forecastQty' },
    { key: 'actualQty', header: 'Actual Value', accessor: (r) => r.actualQty?.toLocaleString(), align: 'right', filterType: 'number', filterField: 'actualQty' },
    {
      ...
      align: 'right', sortable: true, filterType: 'number', filterField: 'mape',
    },
    { key: 'bias', header: 'Bias %', accessor: (r) => r.bias != null ? `${r.bias.toFixed(1)}%` : '—', align: 'right', filterType: 'number', filterField: 'bias' },
    { key: 'trackingSignal', header: 'Tracking Signal', accessor: (r) => r.trackingSignal != null ? r.trackingSignal.toFixed(2) : '—', align: 'right', filterType: 'number', filterField: 'trackingSignal' },
    { key: 'model', header: 'Model Used', accessor: (r) => r.forecastModel || '—', filterType: 'text', filterField: 'forecastModel' },
    {
  ...
  return (
    <div className="space-y-4 lg:space-y-6">
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-xl lg:text-2xl font-bold">Forecast Accuracy</h1>
      ...
      <Card>
        ...
        <DataTable
          ...
          keyExtractor={(r) => r.id}
          isLoading={isLoading}
          emptyMessage="No forecast accuracy metrics found"
