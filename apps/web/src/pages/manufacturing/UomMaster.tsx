...
export default function UomMasterPage() {
  ...
  const grid = useGridState({ initialSortBy: 'code', initialSortOrder: 'asc', initialPageSize: 50 });

  // Queries
  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: ['manufacturing', 'uoms', grid.queryKey],
    queryFn: () => uomService.getAll(grid.queryParams),
  ...
  const columns: Column<UnitOfMeasure>[] = [
    {
      ...
      sortable: true, filterType: 'text', filterField: 'code',
    },
    { key: 'name', header: 'Name', accessor: 'name', sortable: true, filterType: 'text', filterField: 'name' },
    {
      ...
      filterType: 'text', filterField: 'symbol',
    },
    {
      ...
      filterOptions: UOM_CATEGORIES.map((c) => ({ value: c.value, label: c.label })),
    },
    { key: 'decimals', header: 'Decimals', accessor: (r) => r.decimals, align: 'center', filterType: 'number', filterField: 'decimals' },
    {
      ...
      filterType: 'select',
      filterField: 'isBase',
      filterOptions: [{ value: 'true', label: 'Yes' }, { value: 'false', label: 'No' }],
    },
    {
    ...
    { key: 'createdAt', header: 'Created', accessor: (r) => safeFormat(r.createdAt, 'MMM dd, yyyy'), sortable: true },
    {
  ...
  return (
    <div className="space-y-4 lg:space-y-6">
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-xl lg:text-2xl font-bold">UOM Master</h1>
