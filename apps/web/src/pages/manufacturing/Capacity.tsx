...
export default function CapacityPage() {
  ...
  const wcColumns: Column<WorkCenter>[] = [
    ...
    { key: 'cost', header: 'INR/hr', accessor: (r: any) => r.costPerHour != null ? formatInr(r.costPerHour) : '—', align: 'right' },
    ...
    <div className="space-y-4 lg:space-y-6">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-xs lg:text-sm text-secondary-500 mt-1">Work centers, shifts, utilization, and bottleneck analysis</p>
        </div>
      ...
      <Modal isOpen={showCreateWC} onClose={() => setShowCreateWC(false)} title="Create Work Center" size="lg">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 lg:gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Cost INR/hr</label>
      ...
      <Modal isOpen={showDetailWC} onClose={() => { setShowDetailWC(false); setSelectedWC(null); }} title={selectedWC ? `${selectedWC.code} — ${selectedWC.name}` : 'Work Center'} size="xl">
        {selectedWC && (
          <div className="space-y-4 lg:space-y-6">
            <DetailPopupActions
              title="Work Center"
              documentNumber={selectedWC.code}
              fields={[
                { label: 'Code', value: selectedWC.code },
                { label: 'Name', value: selectedWC.name },
                { label: 'Type', value: (wcDetail as any)?.type || selectedWC.type },
                { label: 'Cost/hr', value: formatInr((wcDetail as any)?.costPerHour ?? selectedWC.costPerHour ?? 0) },
                { label: 'Efficiency', value: `${Number((wcDetail as any)?.efficiency ?? (wcDetail as any)?.efficiencyPercent ?? selectedWC.efficiencyPercent ?? '—')}%` },
                { label: 'Status', value: (selectedWC as any).isActive ?? ((selectedWC as any).status === 'ACTIVE') ? 'ACTIVE' : 'INACTIVE' },
              ]}
              tables={capItems.length ? [{
                title: 'Capacity Definitions',
                columns: [
                  { key: 'effectiveDate', header: 'Effective' },
                  { key: 'endDate', header: 'End' },
                  { key: 'standardCapacityPerHour', header: 'Std Cap/hr', align: 'right' as const },
                  { key: 'maxCapacityPerHour', header: 'Max Cap/hr', align: 'right' as const },
                  { key: 'availableHoursPerDay', header: 'Hrs/day', align: 'right' as const },
                ],
                rows: capItems.map((c) => ({
                  effectiveDate: safeFormat(c.effectiveDate, 'yyyy-MM-dd'),
                  endDate: safeFormat(c.endDate, 'yyyy-MM-dd'),
                  standardCapacityPerHour: String(c.standardCapacityPerHour),
                  maxCapacityPerHour: String(c.maxCapacityPerHour ?? '—'),
                  availableHoursPerDay: String(c.availableHoursPerDay ?? '—'),
                })),
              }] : []}
            />
            <div className="grid grid-cols-4 gap-4 text-sm">
              <div><span className="font-medium text-gray-500">Type:</span> {(wcDetail as any)?.type || selectedWC.type}</div>
              <div><span className="font-medium text-gray-500">Cost/hr:</span> {formatInr((wcDetail as any)?.costPerHour ?? selectedWC.costPerHour ?? 0)}</div>
