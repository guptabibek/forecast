...
export default function CapacityPlansPage() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: ['manufacturing', 'capacity-plans', grid.queryKey],
    queryFn: () => capacityPlanService.getAll(grid.queryParams),
  ...
  const planColumns: Column<CapacityPlan>[] = [
    { key: 'name', header: 'Name', accessor: 'name', sortable: true, filterType: 'text', filterField: 'name' },
    {
      ...
      filterType: 'select', filterField: 'planType',
      filterOptions: [
        { value: 'RCCP', label: 'RCCP' },
        { value: 'CRP', label: 'CRP' },
    ...
    { key: 'endDate', header: 'End Date', accessor: (r) => safeFormat(r.endDate, 'MMM dd, yyyy'), filterType: 'date', filterField: 'endDate' },
    {
      key: 'granularity', header: 'Granularity', accessor: 'granularity',
      filterType: 'select', filterField: 'granularity',
      filterOptions: [
    ...
    { key: 'planningHorizon', header: 'Horizon', accessor: (r) => String(r.planningHorizon), align: 'right', filterType: 'number', filterField: 'planningHorizon' },
    {
  ...
  return (
    <div className="space-y-4 lg:space-y-6">
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-xl lg:text-2xl font-bold">Capacity Planning</h1>
          <p className="text-xs lg:text-sm text-secondary-500 mt-1">Utilization analysis, bottleneck detection &amp; capacity plans</p>
        </div>
        {activeTab === 'plans' && (
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4">
            <Card>
              <div className="p-4 text-center">
                <p className="text-xs lg:text-sm text-gray-500 uppercase font-medium">Work Centers</p>
                <p className="text-xl lg:text-2xl font-bold mt-1">{wcUtilSummary.length}</p>
              </div>
            </Card>
            <Card>
              <div className="p-4 text-center">
                <p className="text-xs lg:text-sm text-gray-500 uppercase font-medium">Avg Utilization</p>
                <p className={`text-xl lg:text-2xl font-bold mt-1 ${avgUtil > 100 ? 'text-red-600' : avgUtil > 85 ? 'text-amber-600' : 'text-green-600'}`}>{avgUtil.toFixed(1)}%</p>
              </div>
            </Card>
            <Card>
              <div className="p-4 text-center">
                <p className="text-xs lg:text-sm text-gray-500 uppercase font-medium">Overloaded</p>
                <p className={`text-xl lg:text-2xl font-bold mt-1 ${overloadedCount > 0 ? 'text-red-600' : 'text-green-600'}`}>{overloadedCount}</p>
              </div>
            </Card>
            <Card>
              <div className="p-4 text-center">
                <p className="text-xs lg:text-sm text-gray-500 uppercase font-medium">Critical Bottlenecks</p>
                <p className={`text-xl lg:text-2xl font-bold mt-1 ${criticalBottlenecks > 0 ? 'text-red-600' : 'text-green-600'}`}>{criticalBottlenecks}</p>
              </div>
            </Card>
          </div>
        ...
        <>
          ...
          <Card>
            ...
            <DataTable
              ...
              keyExtractor={(r) => r.id}
              isLoading={isLoading}
              emptyMessage="No capacity plans found"
      ...
      <Modal isOpen={showDetail} onClose={() => { setShowDetail(false); setSelectedPlan(null); }} title={selectedPlan ? `Plan: ${selectedPlan.name}` : 'Plan Detail'} size="xl">
        {selectedPlan && (
          <div className="space-y-4 lg:space-y-6">
            <DetailPopupActions
              title="Capacity Plan"
              documentNumber={selectedPlan.name}
              fields={[
                { label: 'Plan Name', value: selectedPlan.name },
                { label: 'Type', value: selectedPlan.planType },
                { label: 'Status', value: selectedPlan.status },
                { label: 'Granularity', value: selectedPlan.granularity },
                { label: 'Start', value: safeFormat(selectedPlan.startDate, 'MMM dd, yyyy') },
                { label: 'End', value: safeFormat(selectedPlan.endDate, 'MMM dd, yyyy') },
                { label: 'Horizon', value: `${selectedPlan.planningHorizon} periods` },
              ]}
            />
