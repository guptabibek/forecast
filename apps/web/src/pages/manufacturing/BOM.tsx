...
export default function BOMPage() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    ...
    queryFn: () => bomService.getBOMs(grid.queryParams),
  ...
  const columns: Column<BOM>[] = [
    ...
    { key: 'name', header: 'Name', accessor: (row: any) => row.name || '—', sortable: true, filterType: 'text', filterField: 'name' },
    { key: 'version', header: 'Version', accessor: (row: any) => row.version || row.revision || '—', filterType: 'text', filterField: 'version' },
    {
      key: 'status',
      header: 'Status',
      accessor: (row) => (
    ...
    },
    {
      ...
      sortable: true,
      accessor: (row: any) => {
        const d = row.effectiveFrom || row.effectiveDate;
...
return (
    <div className="space-y-4 lg:space-y-6">
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-xl lg:text-2xl font-bold">Bill of Materials</h1>
      ...
      <Card>
        <CardHeader
          title="BOMs"
          description="Filter per column · Sort by clicking header · Server-side"
        />
        <DataTable
          data={items}
          columns={columns}
