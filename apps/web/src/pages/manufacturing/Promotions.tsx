...
export default function PromotionsPage() {
  ...
  // Grid state for the "all" tab — column filters + sort + pagination
  const grid = useGridState({ initialSortBy: 'startDate', initialSortOrder: 'desc' });

  // Queries
  const { data, isLoading, isError: isAllError, error: allError } = useQuery({
    queryKey: ['manufacturing', 'promotions', grid.queryKey],
    queryFn: () => promotionService.getPromotions(grid.queryParams),
    enabled: tab === 'all',
  ...
  const columns: Column<Promotion>[] = [
    { key: 'code', header: 'Code', accessor: 'code', sortable: true, filterType: 'text', filterField: 'name' },
    { key: 'name', header: 'Name', accessor: 'name', sortable: true, filterType: 'text', filterField: 'name' },
    {
      key: 'type', header: 'Type', accessor: 'type',
      filterType: 'select', filterField: 'type',
      filterOptions: PROMO_TYPES.map((t) => ({ value: t, label: t })),
    },
    {
      key: 'status', header: 'Status',
      accessor: (r) => <Badge variant={statusVariant[r.status] || 'secondary'} size="sm">{r.status}</Badge>,
      filterType: 'select', filterField: 'status',
      filterOptions: PROMO_STATUSES.map((s) => ({ value: s, label: s })),
    },
    { key: 'start', header: 'Start', accessor: (r) => safeFormat(r.startDate, 'yyyy-MM-dd'), sortable: true, filterType: 'date', filterField: 'startDate' },
    { key: 'end', header: 'End', accessor: (r) => safeFormat(r.endDate, 'yyyy-MM-dd'), sortable: true, filterType: 'date', filterField: 'endDate' },
    { key: 'discount', header: 'Discount', accessor: (r) => r.discountPercent ? `${r.discountPercent}%` : r.discountAmount ? formatInr(r.discountAmount) : '—', align: 'right', filterType: 'number', filterField: 'discountValue' },
    {
  ...
  return (
    <div className="space-y-4 lg:space-y-6">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3">
        <div>
          <h1 className="text-xl lg:text-2xl font-bold">Promotions</h1>
          <p className="text-xs lg:text-sm text-secondary-500 mt-1">Promotional calendar and forecast overlays</p>
        </div>
      ...
      <Card>
        ...
        <DataTable
          ...
          columns={columns}
          keyExtractor={(r) => r.id}
          isLoading={tab === 'active' ? activeLoading : tab === 'upcoming' ? upcomingLoading : isLoading}
          emptyMessage="No promotions found"
          {...(tab === 'all' ? {
            sorting: grid.sortingProps,
            filtering: grid.filteringProps,
            pagination: grid.paginationProps((data as any)?.total ?? items.length),
          } : {})}
        />
      </Card>
      ...
      <Modal isOpen={showCreate} onClose={() => setShowCreate(false)} title="Create Promotion" size="lg">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Discount INR</label><input type="number" className="w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500" value={form.discountAmount} onChange={(e) => setForm({ ...form, discountAmount: +e.target.value })} /></div>
          ...
          <div className="space-y-4 lg:space-y-6">
            <DetailPopupActions
              ...
              fields={[
                ...
                { label: 'Start', value: safeFormat(selected.startDate, 'MMM d, yyyy') },
                { label: 'End', value: safeFormat(selected.endDate, 'MMM d, yyyy') },
                { label: 'Discount', value: selected.discountPercent ? `${selected.discountPercent}%` : selected.discountAmount ? formatInr(selected.discountAmount) : '—' },
              ]}
              tables={liftItems.length ? [{
                title: 'Lift Factors',
                columns: [
                  { key: 'product', header: 'Product' },
                  { key: 'weekNumber', header: 'Week', align: 'right' as const },
                  { key: 'liftPercent', header: 'Lift %', align: 'right' as const },
                  { key: 'cannibalizationPercent', header: 'Cannib %', align: 'right' as const },
                  { key: 'haloPercent', header: 'Halo %', align: 'right' as const },
                  { key: 'baselineForecast', header: 'Baseline', align: 'right' as const },
                  { key: 'adjustedForecast', header: 'Adjusted', align: 'right' as const },
                ],
                rows: liftItems.map((l) => ({
                  product: l.product?.sku || l.productId,
                  weekNumber: String(l.weekNumber),
                  liftPercent: `${l.liftPercent}%`,
                  cannibalizationPercent: `${l.cannibalizationPercent ?? '—'}%`,
                  haloPercent: `${l.haloPercent ?? '—'}%`,
                  baselineForecast: l.baselineForecast?.toLocaleString() ?? '—',
                  adjustedForecast: l.adjustedForecast?.toLocaleString() ?? '—',
                })),
              }] : []}
            />
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
              ...
              <div><span className="font-medium text-gray-500">Discount:</span> {selected.discountPercent ? `${selected.discountPercent}%` : selected.discountAmount ? formatInr(selected.discountAmount) : '—'}</div>
            </div>
