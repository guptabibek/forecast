...
export default function NPIPage() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: ['manufacturing', 'npi', grid.queryKey],
    queryFn: () => npiService.getNPIs(grid.queryParams),
  ...
  const columns: Column<NewProductIntroduction>[] = [
    { key: 'sku', header: 'SKU', accessor: (r) => r.sku || r.product?.sku || '—', sortable: true, filterType: 'text', filterField: 'sku' },
    { key: 'name', header: 'Name', accessor: (r) => r.name || r.product?.name || '—', sortable: true, filterType: 'text', filterField: 'name' },
    { key: 'category', header: 'Category', accessor: (r) => r.category || '—', filterType: 'text', filterField: 'category' },
    { key: 'brand', header: 'Brand', accessor: (r) => r.brand || '—', filterType: 'text', filterField: 'brand' },
    { key: 'launch', header: 'Launch', accessor: (r) => safeFormat(r.launchDate, 'yyyy-MM-dd'), sortable: true, filterType: 'date', filterField: 'launchDate' },
    {
      key: 'curve', header: 'Curve', accessor: (r) => r.launchCurveType || '—',
      filterType: 'select', filterField: 'launchCurveType',
      filterOptions: CURVE_TYPES.map((c) => ({ value: c, label: c })),
    },
    {
      key: 'status', header: 'Status',
      accessor: (r) => <Badge variant={statusVariant[r.status] || 'secondary'} size="sm">{r.status}</Badge>,
      filterType: 'select', filterField: 'status',
      filterOptions: NPI_STATUSES.map((s) => ({ value: s, label: s })),
    },
    {
  ...
  return (
    <div className="space-y-4 lg:space-y-6">
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-xl lg:text-2xl font-bold">New Product Introduction</h1>
