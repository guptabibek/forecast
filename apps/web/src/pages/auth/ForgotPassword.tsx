...
export default function ForgotPassword() {
  ...
  const onOtpSubmit = async (data: OtpResetForm) => {
    ...
    try {
      ...
      const msg = maybeResponse.response?.data?.message || 'Invalid or expired OTP';
      toast.error(msg);
    } finally {
      setIsLoading(false);
    }
    ...
    return (
      <div>
        <div className="bg-white dark:bg-secondary-800 rounded-2xl shadow-xl p-8">
          ...
          <div className="mb-8">
            <h2 className="text-xl lg:text-2xl font-bold">Enter verification code</h2>
  ...
  return (
    <div>
      <div className="bg-white dark:bg-secondary-800 rounded-2xl shadow-xl p-8">
        ...
        <div className="mb-8">
          <h2 className="text-xl lg:text-2xl font-bold">Forgot your password?</h2>
          <p className="text-secondary-500 mt-2">Enter your email and we'll send you a verification code.</p>
        </div>

        <form onSubmit={emailForm.handleSubmit(onEmailSubmit)} className="space-y-4 lg:space-y-6">
          <div>
