...
const settingsSchema = z.object({
  ...
  aiReporting: z.object({
    enabled: z.boolean().optional(),
    summariesEnabled: z.boolean().optional(),
    maxRows: z.number().min(1).max(1000).optional(),
    monthlyUsageCap: z.number().min(1).max(1000000).optional(),
    maskSensitiveFields: z.boolean().optional(),
...
const tabs = [
  ...
  { key: 'ai-reporting', label: 'AI Reporting', icon: SparklesIcon },
  ...
  const {
    ...
    values: settings
      ? {
          ...
          aiReporting: {
            enabled: settings.aiReporting?.enabled ?? false,
            summariesEnabled: settings.aiReporting?.summariesEnabled ?? true,
            maxRows: settings.aiReporting?.maxRows ?? 500,
            monthlyUsageCap: settings.aiReporting?.monthlyUsageCap ?? 5000,
            maskSensitiveFields: settings.aiReporting?.maskSensitiveFields ?? true,
            allowedRoles: settings.aiReporting?.allowedRoles ?? [],
          },
        }
      : undefined,
  });
          ...
          <div className="flex-1">
            <div className="card p-6 space-y-6">
              ...
              {activeTab === 'ai-reporting' && (
                <>
                  <div>
                    <h2 className="text-lg font-semibold mb-2">AI Reporting Controls</h2>
                    <p className="text-sm text-secondary-500 mb-6">
                      Configure tenant-level limits for natural-language reporting. The environment feature flag must also be enabled.
                    </p>
                    {settings?.aiReporting?.environmentEnabled === false && (
                      <div className="mb-5 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                        AI reporting is disabled by the server environment flag. Tenant settings are saved but cannot enable the feature until operations sets AI_REPORTING_ENABLED=true.
                      </div>
                    )}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                      ...
                      <div>
                        ...
                        <input type="number" min={1} max={1000} {...register('aiReporting.maxRows', { valueAsNumber: true })} className="input w-full" />
                        <p className="text-xs text-secondary-400 mt-1">Capped by the server environment maximum.</p>
                      </div>

                      <div>
                        <label className="label">Monthly company usage cap</label>
                        <input type="number" min={1} max={1000000} {...register('aiReporting.monthlyUsageCap', { valueAsNumber: true })} className="input w-full" />
                        <p className="text-xs text-secondary-400 mt-1">Operational limit for AI calls per company.</p>
                      </div>
                      ...
                      <label className="flex items-center justify-between rounded-lg border border-secondary-200 p-4 md:col-span-2">
                        <span>
                          ...
                          <span className="block text-xs text-secondary-500">Exclude PAN, VAT, GST, phone, email, address, bank, and token-like fields from normal AI output.</span>
                        </span>
                        <input type="checkbox" {...register('aiReporting.maskSensitiveFields')} className="h-4 w-4 rounded border-secondary-300 text-primary-600" />
                      </label>
                    </div>
