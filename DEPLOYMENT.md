...
| `AI_REPORTING_ENABLED` | No | Feature flag for AI NLQ reporting. Per-tenant provider credentials live in `ai_tenant_provider_configs` and are managed in Settings → AI Reporting. |
| `AI_CONFIG_ENCRYPTION_KEY` | No | 32+ char key for encrypting tenant-stored API keys. Falls back to `JWT_SECRET` if unset. |
