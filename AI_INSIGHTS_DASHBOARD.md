...
| 8 | High | "Top 5 routes/cities/areas/states with most sales/purchase" answered as ungrouped "top 5 records": the catalog had NO regional dimension (the views' route_code/area_code come from the deprecated mutable party master), and nothing caught the LLM dropping the grouping | Migration `20260611130000_ai_views_regional_dimensions` adds `region_route_*`/`region_area_*` to the net views from the canonical transaction-time source (`add_field` seg 20/21 + `marg_sale_types` ROUT/AREA — same as the dashboard regional reports; dry-run verified against the live schema). Catalog v1.4 registers route/area dimensions (+state/region/territory/city synonyms) and brand/distributor synonyms. |
| 9 | High | No deterministic guard for grouping/ranking intent — every "top N <entity>" question rode entirely on the LLM | New `grouping-intent.util.ts` (pure, extensible): detects ranking intent/direction/limit, extracts the ranked noun, resolves it against catalog dimension synonyms; repairs dimensionless LLM output into a grouped ranking query (incl. sales_items→sales_net sibling remap), or returns a precise `unsupported` (`MISSING_GROUPING_DIMENSION`) when a known business entity has no dimension. Wired into `NlqParserService`; prompt rules 9a/9b added (prompt v1.0.6). 25 unit tests against the real catalog. |
| 10 | Medium | Percentage metrics rendered as currency (₹25.40 instead of 25.4%): the client formatter had no percentage branch and its currency key-regex overrode the catalog dataType | `formatAiValue` is now dataType-authoritative (percentage/currency/number/quantity) with percent-key heuristics before currency fallback — covers tables, charts, KPI cards, and pinned widgets through the one shared formatter. CSV exports intentionally stay raw-numeric. |
| 11 | Medium | Pinned widgets showed raw results only — no growth, contribution, trend, or narrative reading | `result-analytics.util.ts` + `WidgetExecutorService.buildAnalytics`: every widget execution now carries an additive `analytics` block (total/average/rows KPIs, growth vs equal-length previous window for past-facing custom ranges — at most ONE extra query, cached with the payload — top-contributor share, top-3 share, Pareto-80% count, monthly trend direction, narrative insight lines). Rendered as a KPI-chip + findings strip on WidgetCard; previous-window construction shared with the pinned-report insight provider. |
| 12 | Low | WidgetCard diverged from the design system: raw gray classes, no dark-mode variants, no hover treatment | Restyled with secondary-* design tokens + dark variants throughout, hover elevation, dark skeletons, and an explicit "AI" sparkles badge so pinned cards match native cards but stay labeled. |
| 13 | High | "Top 10 items whose sales decreased vs previous month" produced two stacked period lists sorted by lowest sellers — ranking by period-over-period CHANGE did not exist anywhere in the engine | New change-ranking pipeline: `comparison.rankBy='change'` semantics → `SqlCompilerService.compileChangeRanking` aggregates both periods per dimension value, FULL-OUTER-JOINs them (null-safe, so vanished/new items still rank), emits current/previous/`change`/`change_pct` typed columns, and orders by the signed delta (asc = biggest decreases). Prompt rule 9c (v1.0.7) + deterministic `repairChangeRankingIntent` backstop that detects decrease/increase-vs-previous-period phrasing, resolves the dimension, sets direction and limit. Net datasets added to the comparison-capable set. |
